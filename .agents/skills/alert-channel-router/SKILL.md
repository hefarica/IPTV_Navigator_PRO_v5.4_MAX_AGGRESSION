---
name: alert-channel-router
description: Use when routing alerts (email/Slack/Telegram/Pushover) per severity (info/warn/critical) with rate limit + dedup.
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when routing alerts (email/Slack/Telegram/Pushover) per severity (info/warn/critical) with rate limit + dedup.
