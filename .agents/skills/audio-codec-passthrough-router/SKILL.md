---
name: audio-codec-passthrough-router
description: Use when deciding `passthrough` (Atmos/DD+/DTS-X) vs `transcode to AAC-LC` per player capability matrix.
specialist: S3
kind: satellite
parent: codec-quality-analyzer
---

# audio-codec-passthrough-router

## Cuándo usarla
Use when deciding `passthrough` (Atmos/DD+/DTS-X) vs `transcode to AAC-LC` per player capability matrix.
