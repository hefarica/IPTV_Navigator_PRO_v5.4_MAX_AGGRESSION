...
description: Use when implementing closed/open/half-open circuit breaker — but RESPECT autopista doctrine (do NOT add breaker for single-user IPTV).
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when implementing closed/open/half-open circuit breaker — but RESPECT autopista doctrine (do NOT add breaker for single-user IPTV).
