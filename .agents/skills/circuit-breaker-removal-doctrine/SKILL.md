---
name: circuit-breaker-removal-doctrine
description: Use when REMOVING circuit breaker logic per autopista doctrine — Lua reactor must be passthrough/telemetry-only, never ngx.exit(503).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# circuit-breaker-removal-doctrine

## Cuándo usarla
Use when REMOVING circuit breaker logic per autopista doctrine — Lua reactor must be passthrough/telemetry-only, never ngx.exit(503).
