---
name: cmaf-init-segment-validator
description: Use when validating CMAF init segment (`init.mp4` or `*.mp4`) contains `ftyp` + `moov` with required boxes (mvhd, trak/tkhd/mdia).
specialist: S2
kind: satellite
parent: ll-hls-cmaf-engineer
---

# cmaf-init-segment-validator

## Cuándo usarla
Use when validating CMAF init segment (`init.mp4` or `*.mp4`) contains `ftyp` + `moov` with required boxes (mvhd, trak/tkhd/mdia).
