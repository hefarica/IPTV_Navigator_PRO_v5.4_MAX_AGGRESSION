---
name: cmaf-styp-stable-marker
description: Use when emitting `styp` box with brand=`cmfc` / `cmfs` to mark stable CMAF chunks.
specialist: S2
kind: satellite
parent: ll-hls-cmaf-engineer
---

# cmaf-styp-stable-marker

## Cuándo usarla
Use when emitting `styp` box with brand=`cmfc` / `cmfs` to mark stable CMAF chunks.
