---
name: codec-fallback-selector
description: Use when selecting fallback codec ladder when probe fails (HEVC Main10 PREFERRED → AVC High → AVC fallback) without breaking continuity.
specialist: S3
kind: satellite
parent: codec-quality-analyzer
---

# codec-fallback-selector

## Cuándo usarla
Use when selecting fallback codec ladder when probe fails (HEVC Main10 PREFERRED → AVC High → AVC fallback) without breaking continuity.
