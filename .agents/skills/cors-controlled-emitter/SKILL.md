---
name: cors-controlled-emitter
description: Use when emitting controlled `Access-Control-Allow-Origin: <specific origin>` (NEVER *) for player CORS without exposure.
specialist: S8
kind: satellite
parent: vps-network-tcp-quic-tuning
---

# cors-controlled-emitter

## Cuándo usarla
Use when emitting controlled `Access-Control-Allow-Origin: <specific origin>` (NEVER *) for player CORS without exposure.
