---
name: dependency-audit-npm-pip-pip-audit
description: Use when running `npm audit --production` / `pip-audit` / `safety check` to scan for CVE in deps.
specialist: S10
kind: satellite
parent: secure-header-profiler
---

# dependency-audit-npm-pip-pip-audit

## Cuándo usarla
Use when running `npm audit --production` / `pip-audit` / `safety check` to scan for CVE in deps.
