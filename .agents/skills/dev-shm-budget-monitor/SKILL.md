---
name: dev-shm-budget-monitor
description: Use when monitoring /dev/shm usage (default tmpfs cap 50% RAM; OOM if hot files grow unbounded).
specialist: S7
kind: satellite
parent: stream-watchdog-sre
---

# dev-shm-budget-monitor

## Cuándo usarla
Use when monitoring /dev/shm usage (default tmpfs cap 50% RAM; OOM if hot files grow unbounded).
