---
name: dev-shm-permissions-enforcer
description: Use when enforcing chmod 644 on /dev/shm files written by root daemons (umask 077 default → nginx www-data cannot read → telemetry vacía).
specialist: S7
kind: satellite
parent: stream-watchdog-sre
---

# dev-shm-permissions-enforcer

## Cuándo usarla
Use when enforcing chmod 644 on /dev/shm files written by root daemons (umask 077 default → nginx www-data cannot read → telemetry vacía).
