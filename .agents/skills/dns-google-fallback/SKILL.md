---
name: dns-google-fallback
description: Use when keeping 8.8.8.8 as DNS fallback (lower priority via resolv.conf order).
specialist: S8
kind: satellite
...
## Cuándo usarla
Use when keeping 8.8.8.8 as DNS fallback (lower priority via resolv.conf order).
