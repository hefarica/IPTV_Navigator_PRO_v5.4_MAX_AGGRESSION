---
name: duplicate-stream-deduper
description: Use when deduplicating channels by canonical URL + tvg-id + name fingerprint to prevent multi-emit of the same upstream.
specialist: S1
kind: satellite
parent: m3u8-playlist-surgeon
---

# duplicate-stream-deduper

## Cuándo usarla
Use when deduplicating channels by canonical URL + tvg-id + name fingerprint to prevent multi-emit of the same upstream.
