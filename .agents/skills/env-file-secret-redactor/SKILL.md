---
name: env-file-secret-redactor
description: Use when redacting .env values from logs (replace `=value` with `=***REDACTED***` before write).
specialist: S10
kind: satellite
parent: secure-header-profiler
---

# env-file-secret-redactor

## Cuándo usarla
Use when redacting .env values from logs (replace `=value` with `=***REDACTED***` before write).
