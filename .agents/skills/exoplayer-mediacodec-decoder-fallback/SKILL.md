---
name: exoplayer-mediacodec-decoder-fallback
description: Use when handling MediaCodec decoder fallback (HW → SW) when hardware decoder rejects format.
specialist: S9
kind: satellite
parent: player-compatibility-matrix
---

# exoplayer-mediacodec-decoder-fallback

## Cuándo usarla
Use when handling MediaCodec decoder fallback (HW → SW) when hardware decoder rejects format.
