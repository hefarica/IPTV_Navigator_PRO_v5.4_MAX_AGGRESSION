---
name: ext-x-define-emitter
description: Use when emitting `#EXT-X-DEFINE:NAME=...,VALUE=...` variables (HLS v8+) and substituting `{$NAME}` in URIs.
specialist: S1
kind: satellite
parent: iptv-hls-validator
---

# ext-x-define-emitter

## Cuándo usarla
Use when emitting `#EXT-X-DEFINE:NAME=...,VALUE=...` variables (HLS v8+) and substituting `{$NAME}` in URIs.
