---
name: ext-x-discontinuity-handler
description: Use when marking PTS/DTS discontinuities with `#EXT-X-DISCONTINUITY` between segments from different sources.
specialist: S1
kind: satellite
parent: hls-rfc8216-media-playlist-builder
---

# ext-x-discontinuity-handler

## Cuándo usarla
Use when marking PTS/DTS discontinuities with `#EXT-X-DISCONTINUITY` between segments from different sources.
