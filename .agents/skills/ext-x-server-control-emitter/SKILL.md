---
name: ext-x-server-control-emitter
description: Use when emitting `#EXT-X-SERVER-CONTROL:CAN-BLOCK-RELOAD=YES,PART-HOLD-BACK=...,CAN-SKIP-UNTIL=...` to enable LL-HLS blocking behavior.
specialist: S2
kind: satellite
parent: ll-hls-cmaf-engineer
---

# ext-x-server-control-emitter

## Cuándo usarla
Use when emitting `#EXT-X-SERVER-CONTROL:CAN-BLOCK-RELOAD=YES,PART-HOLD-BACK=...,CAN-SKIP-UNTIL=...` to enable LL-HLS blocking behavior.
...
## Anchor skill
Sub-skill of: **ll-hls-cmaf-engineer** (ver `.agents/skills/ll-hls-cmaf-engineer/SKILL.md` para el marco completo).
...
Hereda del anchor `ll-hls-cmaf-engineer` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `ll-hls-cmaf-engineer` (QoE composite, error rate, compatibilidad player).
