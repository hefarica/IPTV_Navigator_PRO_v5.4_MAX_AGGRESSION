---
name: ext-x-session-key-emitter
description: Use when pre-fetching keys via `#EXT-X-SESSION-KEY` in a master playlist before first segment fetch.
specialist: S1
kind: satellite
parent: hls-rfc8216-master-playlist-builder
---

# ext-x-session-key-emitter

## Cuándo usarla
Use when pre-fetching keys via `#EXT-X-SESSION-KEY` in a master playlist before first segment fetch.
