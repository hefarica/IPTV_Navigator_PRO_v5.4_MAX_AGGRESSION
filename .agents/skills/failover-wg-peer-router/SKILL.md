---
name: failover-wg-peer-router
description: Use when implementing sub-second failover between WG peers (Miami ↔ Brasil) via wg-health-fast + sentinel 407 hook + conntrack flush.
specialist: S8
kind: satellite
parent: vps-network-tcp-quic-tuning
---

# failover-wg-peer-router

## Cuándo usarla
Use when implementing sub-second failover between WG peers (Miami ↔ Brasil) via wg-health-fast + sentinel 407 hook + conntrack flush.
