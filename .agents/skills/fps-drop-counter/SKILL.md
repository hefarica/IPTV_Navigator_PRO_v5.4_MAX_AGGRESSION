---
name: fps-drop-counter
description: Use when counting dropped frames per second via player API (dropped_video_frames) — target <1 fps drops sustained.
specialist: S5
kind: satellite
...
## Cuándo usarla
Use when counting dropped frames per second via player API (dropped_video_frames) — target <1 fps drops sustained.
