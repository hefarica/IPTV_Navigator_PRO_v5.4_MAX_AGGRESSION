---
name: fps-true-vs-declared-checker
description: Use when verifying real FPS (avg_frame_rate) matches declared rate (r_frame_rate) — detects 30→60 interpolation fakes.
specialist: S3
kind: satellite
parent: ffmpeg-ffprobe-validator
---

# fps-true-vs-declared-checker

## Cuándo usarla
Use when verifying real FPS (avg_frame_rate) matches declared rate (r_frame_rate) — detects 30→60 interpolation fakes.
...
## Anchor skill
Sub-skill of: **ffmpeg-ffprobe-validator** (ver `.agents/skills/ffmpeg-ffprobe-validator/SKILL.md` para el marco completo).
...
Hereda del anchor `ffmpeg-ffprobe-validator` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `ffmpeg-ffprobe-validator` (QoE composite, error rate, compatibilidad player).
