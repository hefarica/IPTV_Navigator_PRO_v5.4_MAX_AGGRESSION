---
name: gzip-off-for-video-enforcer
description: Use when enforcing `gzip off` in video locations (gzip on TS/fMP4/MP4 wastes CPU + breaks Range requests).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# gzip-off-for-video-enforcer

## Cuándo usarla
Use when enforcing `gzip off` in video locations (gzip on TS/fMP4/MP4 wastes CPU + breaks Range requests).
