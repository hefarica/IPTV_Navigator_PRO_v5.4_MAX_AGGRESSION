---
name: h2-server-push-helper
description: Use when emitting `http2_push /init.mp4;` or Link rel=preload headers for LL-HLS init+first part push.
specialist: S8
kind: satellite
parent: vps-network-tcp-quic-tuning
---

# h2-server-push-helper

## Cuándo usarla
Use when emitting `http2_push /init.mp4;` or Link rel=preload headers for LL-HLS init+first part push.
