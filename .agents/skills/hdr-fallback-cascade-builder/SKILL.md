---
name: hdr-fallback-cascade-builder
description: Use when building HDR→SDR fallback chain (DV → HDR10+ → HDR10 → HLG → SDR BT.709) per player capability.
specialist: S4
kind: satellite
parent: hdr-color-science-architect
---

# hdr-fallback-cascade-builder

## Cuándo usarla
Use when building HDR→SDR fallback chain (DV → HDR10+ → HDR10 → HLG → SDR BT.709) per player capability.
