---
name: hdr-mismatch-alert
description: Use when alerting on HDR mismatch (e.g., DV declared but no `dvcC` box present, or BT.2020 with bt709 fallback).
specialist: S4
kind: satellite
...
## Cuándo usarla
Use when alerting on HDR mismatch (e.g., DV declared but no `dvcC` box present, or BT.2020 with bt709 fallback).
