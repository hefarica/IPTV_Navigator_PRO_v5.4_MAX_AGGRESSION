---
name: header-filter-by-lua-helper
description: Use when mutating response headers in `header_filter_by_lua_file` phase (after upstream response, before body sent).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# header-filter-by-lua-helper

## Cuándo usarla
Use when mutating response headers in `header_filter_by_lua_file` phase (after upstream response, before body sent).
