---
name: http2-enabler
description: Use when enabling HTTP/2 on nginx via `listen 443 ssl http2;` (h2 over TLS, mandatory ALPN).
specialist: S8
kind: satellite
...
## Cuándo usarla
Use when enabling HTTP/2 on nginx via `listen 443 ssl http2;` (h2 over TLS, mandatory ALPN).
