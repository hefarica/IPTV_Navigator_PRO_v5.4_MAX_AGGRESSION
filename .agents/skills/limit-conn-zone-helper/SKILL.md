---
name: limit-conn-zone-helper
description: Use when defining `limit_conn_zone $binary_remote_addr zone=Y:10m` and `limit_conn Y N` to cap concurrent connections.
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# limit-conn-zone-helper

## Cuándo usarla
Use when defining `limit_conn_zone $binary_remote_addr zone=Y:10m` and `limit_conn Y N` to cap concurrent connections.
