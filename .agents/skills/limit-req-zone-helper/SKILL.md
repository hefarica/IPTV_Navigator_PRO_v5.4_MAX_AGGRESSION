---
name: limit-req-zone-helper
description: Use when defining `limit_req_zone $binary_remote_addr zone=X:10m rate=Yr/s` and `limit_req zone=X burst=Z` per location.
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# limit-req-zone-helper

## Cuándo usarla
Use when defining `limit_req_zone $binary_remote_addr zone=X:10m rate=Yr/s` and `limit_req zone=X burst=Z` per location.
