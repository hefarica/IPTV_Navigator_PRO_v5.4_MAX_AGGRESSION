---
name: log-by-lua-helper-non-blocking
description: Use when running telemetry in `log_by_lua_file` phase (AFTER response sent, zero blocking impact on client).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# log-by-lua-helper-non-blocking

## Cuándo usarla
Use when running telemetry in `log_by_lua_file` phase (AFTER response sent, zero blocking impact on client).
