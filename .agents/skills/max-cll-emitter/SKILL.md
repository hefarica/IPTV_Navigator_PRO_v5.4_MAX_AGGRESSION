---
name: max-cll-emitter
description: Use when emitting MaxCLL (Maximum Content Light Level) in HDR10 static metadata (cd/m² peak per pixel).
specialist: S4
kind: satellite
...
## Cuándo usarla
Use when emitting MaxCLL (Maximum Content Light Level) in HDR10 static metadata (cd/m² peak per pixel).
