---
name: nginx-config-snapshot-helper
description: Use when snapshotting live config (`nginx -T > /tmp/nginx-pre-$(date +%s).conf`) as byte-exact rollback baseline.
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# nginx-config-snapshot-helper

## Cuándo usarla
Use when snapshotting live config (`nginx -T > /tmp/nginx-pre-$(date +%s).conf`) as byte-exact rollback baseline.
