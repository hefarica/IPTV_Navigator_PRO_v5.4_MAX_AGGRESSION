---
name: nginx-syntax-validator-nginx-t
description: Use when validating nginx config with `nginx -t` before reload — REQUIRED step before any deploy.
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# nginx-syntax-validator-nginx-t

## Cuándo usarla
Use when validating nginx config with `nginx -t` before reload — REQUIRED step before any deploy.
