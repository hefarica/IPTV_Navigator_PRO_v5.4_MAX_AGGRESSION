---
name: ngx-shared-dict-helper
description: Use when defining `lua_shared_dict name 10m` for cross-worker state (telemetry, circuit metrics, config cache).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# ngx-shared-dict-helper

## Cuándo usarla
Use when defining `lua_shared_dict name 10m` for cross-worker state (telemetry, circuit metrics, config cache).
