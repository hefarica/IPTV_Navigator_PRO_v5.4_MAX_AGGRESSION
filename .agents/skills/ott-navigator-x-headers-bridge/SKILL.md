---
name: ott-navigator-x-headers-bridge
description: Use when bridging X-APE-* custom headers from m3u8 → OTT Navigator settings (note: OTT may not parse non-standard tags per RFC 8216 §6.3.1).
specialist: S9
kind: satellite
parent: android-tv-ott-tivimate-tuning
---

# ott-navigator-x-headers-bridge

## Cuándo usarla
Use when bridging X-APE-* custom headers from m3u8 → OTT Navigator settings (note: OTT may not parse non-standard tags per RFC 8216 §6.3.1).
