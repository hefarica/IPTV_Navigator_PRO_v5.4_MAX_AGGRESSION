---
name: peak-bitrate-emitter
description: Use when emitting `BANDWIDTH` attribute (peak bitrate over any 1-second window per HLS spec).
specialist: S5
kind: satellite
parent: qoe-observability-engine
---

# peak-bitrate-emitter

## Cuándo usarla
Use when emitting `BANDWIDTH` attribute (peak bitrate over any 1-second window per HLS spec).
