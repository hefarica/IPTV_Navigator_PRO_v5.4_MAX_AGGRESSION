...
description: Use when checking `ss -tln | grep :PORT` for listener presence and probing with curl/nc.
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when checking `ss -tln | grep :PORT` for listener presence and probing with curl/nc.
