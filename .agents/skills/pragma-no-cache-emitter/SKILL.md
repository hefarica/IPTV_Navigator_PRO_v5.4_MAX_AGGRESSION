---
name: pragma-no-cache-emitter
description: Use when emitting `Pragma: no-cache` for HTTP/1.0 cache compat alongside Cache-Control.
specialist: S10
kind: satellite
parent: secure-header-profiler
---

# pragma-no-cache-emitter

## Cuándo usarla
Use when emitting `Pragma: no-cache` for HTTP/1.0 cache compat alongside Cache-Control.
