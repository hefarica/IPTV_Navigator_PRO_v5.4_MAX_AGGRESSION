...
description: Use when implementing pidfile + `pgrep -f process_name` health check with restart-on-zombie.
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when implementing pidfile + `pgrep -f process_name` health check with restart-on-zombie.
