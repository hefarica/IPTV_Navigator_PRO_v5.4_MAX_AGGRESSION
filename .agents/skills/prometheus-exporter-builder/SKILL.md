...
description: Use when emitting Prometheus `/metrics` endpoint with stream counters (TYPE counter / TYPE gauge / TYPE histogram).
specialist: S5
kind: satellite
...
## Cuándo usarla
Use when emitting Prometheus `/metrics` endpoint with stream counters (TYPE counter / TYPE gauge / TYPE histogram).
