...
description: Use when probing per-provider health (Xtream login + canal probe + error rate) with cooldown on failure.
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when probing per-provider health (Xtream login + canal probe + error rate) with cooldown on failure.
