---
name: proxy-cache-key-builder
description: Use when building `proxy_cache_key` ($scheme$proxy_host$uri$is_args$args; exclude credentials, time-vary tokens).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# proxy-cache-key-builder

## Cuándo usarla
Use when building `proxy_cache_key` ($scheme$proxy_host$uri$is_args$args; exclude credentials, time-vary tokens).
