---
name: proxy-cache-path-validator
description: Use when validating `proxy_cache_path` placement (/dev/shm for hot, with full nginx RESTART required for path changes — reload SILENTLY fails).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# proxy-cache-path-validator

## Cuándo usarla
Use when validating `proxy_cache_path` placement (/dev/shm for hot, with full nginx RESTART required for path changes — reload SILENTLY fails).
