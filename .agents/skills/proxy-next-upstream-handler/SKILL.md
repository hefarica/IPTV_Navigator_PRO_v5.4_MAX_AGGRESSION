---
name: proxy-next-upstream-handler
description: Use when configuring `proxy_next_upstream error timeout http_500 http_502 http_503 http_504` (NEVER on 4xx — would mask provider issues).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# proxy-next-upstream-handler

## Cuándo usarla
Use when configuring `proxy_next_upstream error timeout http_500 http_502 http_503 http_504` (NEVER on 4xx — would mask provider issues).
