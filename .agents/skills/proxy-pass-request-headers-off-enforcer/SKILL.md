---
name: proxy-pass-request-headers-off-enforcer
description: Use when enforcing `proxy_pass_request_headers off` in shield locations to prevent 30+ X-* garbage headers from leaking to upstream (fingerprint → 401).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# proxy-pass-request-headers-off-enforcer

## Cuándo usarla
Use when enforcing `proxy_pass_request_headers off` in shield locations to prevent 30+ X-* garbage headers from leaking to upstream (fingerprint → 401).

## Especialista
**Nginx/OpenResty/Lua Engineer** — Edge/Proxy.
Domain mission: nginx -t, Lua phases, passthrough doctrine, no destructive cache.

## Anchor skill
Sub-skill of: **nginx-openresty-streaming-proxy** (ver `.agents/skills/nginx-openresty-streaming-proxy/SKILL.md` para el marco completo).
...
Hereda del anchor `nginx-openresty-streaming-proxy` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `nginx-openresty-streaming-proxy` (QoE composite, error rate, compatibilidad player).
