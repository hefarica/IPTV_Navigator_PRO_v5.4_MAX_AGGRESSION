---
name: proxy-set-header-sanitizer
description: Use when reviewing every `proxy_set_header` directive for toxic headers (Range, If-None-Match, etc.).
specialist: S6
kind: satellite
parent: nginx-openresty-streaming-proxy
---

# proxy-set-header-sanitizer

## Cuándo usarla
Use when reviewing every `proxy_set_header` directive for toxic headers (Range, If-None-Match, etc.).
