---
name: rate-limit-anti-abuse-builder
description: Use when implementing rate limit per IP / per token to prevent abuse without harming legitimate single-user IPTV (respect autopista doctrine).
specialist: S10
kind: satellite
parent: secure-header-profiler
---

# rate-limit-anti-abuse-builder

## Cuándo usarla
Use when implementing rate limit per IP / per token to prevent abuse without harming legitimate single-user IPTV (respect autopista doctrine).

## Especialista
**Security/Auth/Headers Engineer** — Security.
Domain mission: Toxic header blocker, signed URLs, secret scanner, CORS, rate limit.

## Anchor skill
Sub-skill of: **secure-header-profiler** (ver `.agents/skills/secure-header-profiler/SKILL.md` para el marco completo).
...
Hereda del anchor `secure-header-profiler` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `secure-header-profiler` (QoE composite, error rate, compatibilidad player).
