---
name: resolution-vs-bitrate-coherence-checker
description: Use when detecting `4K @ 3 Mbps` upscale fakes by cross-checking resolution against minimum viable bitrate per H.265 reference.
specialist: S3
kind: satellite
parent: codec-quality-analyzer
---

# resolution-vs-bitrate-coherence-checker

## Cuándo usarla
Use when detecting `4K @ 3 Mbps` upscale fakes by cross-checking resolution against minimum viable bitrate per H.265 reference.
...
## Anchor skill
Sub-skill of: **codec-quality-analyzer** (ver `.agents/skills/codec-quality-analyzer/SKILL.md` para el marco completo).
...
Hereda del anchor `codec-quality-analyzer` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `codec-quality-analyzer` (QoE composite, error rate, compatibilidad player).
