---
name: secure-header-profiler
description: Per-player header allowlist/blocklist matrix. EXTHTTP trap detector. Header sanitization, OkHttp single-value rules, 4-layer fallback comma-separated rules, anti-fingerprint surface for shield.
specialist: S10
kind: anchor
---

# secure-header-profiler

## 1. Nombre de la skill
secure-header-profiler

## 2. Cuándo usarla
Per-player header allowlist/blocklist matrix. EXTHTTP trap detector. Header sanitization, OkHttp single-value rules, 4-layer fallback comma-separated rules, anti-fingerprint surface for shield.

Anchored to specialist: **Security/Auth/Headers Engineer** (S-tag S10).
Mission: HTTP headers, EXTHTTP traps, OkHttp single-value, 4-layer fallback
