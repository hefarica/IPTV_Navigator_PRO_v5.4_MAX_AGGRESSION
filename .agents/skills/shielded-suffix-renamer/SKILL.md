---
name: shielded-suffix-renamer
description: Use when renaming output file to `_SHIELDED.m3u8` (filename-only treatment per VPS shield doctrine). NEVER transform inner channel URLs.
specialist: S1
kind: satellite
parent: m3u8-playlist-surgeon
---

# shielded-suffix-renamer

## Cuándo usarla
Use when renaming output file to `_SHIELDED.m3u8` (filename-only treatment per VPS shield doctrine). NEVER transform inner channel URLs.
...
## Anchor skill
Sub-skill of: **m3u8-playlist-surgeon** (ver `.agents/skills/m3u8-playlist-surgeon/SKILL.md` para el marco completo).
...
Hereda del anchor `m3u8-playlist-surgeon` y del set permitido por la doctrina IPTV.
...
## Métricas que mejora
Sub-set de las métricas del anchor `m3u8-playlist-surgeon` (QoE composite, error rate, compatibilidad player).
