---
name: signed-url-hmac-builder
description: Use when generating HMAC-SHA256 signed URLs `?token=<hex>&expires=<ts>` for time-limited authorized access.
specialist: S10
kind: satellite
parent: secure-header-profiler
---

# signed-url-hmac-builder

## Cuándo usarla
Use when generating HMAC-SHA256 signed URLs `?token=<hex>&expires=<ts>` for time-limited authorized access.
