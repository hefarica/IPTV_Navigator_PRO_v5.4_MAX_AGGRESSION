---
name: single-url-per-channel-enforcer
description: Use when enforcing exactly ONE URL per `#EXTINF` block to prevent HTTP 509 Bandwidth Limit Exceeded from upstreams with max_connections=1.
specialist: S1
kind: satellite
parent: iptv-hls-validator
---

# single-url-per-channel-enforcer

## Cuándo usarla
Use when enforcing exactly ONE URL per `#EXTINF` block to prevent HTTP 509 Bandwidth Limit Exceeded from upstreams with max_connections=1.
