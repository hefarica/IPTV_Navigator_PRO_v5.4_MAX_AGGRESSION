---
name: stall-count-tracker
description: Use when counting playback stall events (buffer=0 → buffer≥playback_threshold) over a session.
specialist: S5
kind: satellite
...
## Cuándo usarla
Use when counting playback stall events (buffer=0 → buffer≥playback_threshold) over a session.
