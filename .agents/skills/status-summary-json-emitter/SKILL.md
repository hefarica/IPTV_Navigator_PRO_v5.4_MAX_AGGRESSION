---
name: status-summary-json-emitter
description: Use when emitting /status.json with {channels: [{id, name, last_ok, error_rate}], providers, system}.
specialist: S7
kind: satellite
parent: stream-watchdog-sre
---

# status-summary-json-emitter

## Cuándo usarla
Use when emitting /status.json with {channels: [{id, name, last_ok, error_rate}], providers, system}.
