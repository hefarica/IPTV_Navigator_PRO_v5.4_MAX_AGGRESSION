---
name: stream-watchdog-sre
description: Watchdog per process/port/endpoint/channel/provider. Auto-restart with backoff, circuit-breaker (telemetry-only per autopista), failover policies, log rotation, status JSON, 365-day runbook.
specialist: S7
kind: anchor
---
...
## 2. Cuándo usarla
Watchdog per process/port/endpoint/channel/provider. Auto-restart with backoff, circuit-breaker (telemetry-only per autopista), failover policies, log rotation, status JSON, 365-day runbook.

Anchored to specialist: **Linux VPS/SRE Engineer** (S-tag S7).
Mission: SRE patterns, systemd, watchdog, healthcheck, runbooks
