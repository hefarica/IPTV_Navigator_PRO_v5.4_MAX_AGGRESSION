---
name: sysctl-tcp-bbr-enabler
description: Use when enabling TCP BBR via `net.core.default_qdisc=fq` + `net.ipv4.tcp_congestion_control=bbr` (kernel 4.9+).
specialist: S7
kind: satellite
parent: stream-watchdog-sre
---

# sysctl-tcp-bbr-enabler

## Cuándo usarla
Use when enabling TCP BBR via `net.core.default_qdisc=fq` + `net.ipv4.tcp_congestion_control=bbr` (kernel 4.9+).
