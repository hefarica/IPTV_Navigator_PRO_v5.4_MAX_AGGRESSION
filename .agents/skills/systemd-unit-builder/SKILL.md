...
description: Use when writing `/etc/systemd/system/X.service` unit with [Unit]/[Service]/[Install] sections, Type=, Restart=, RestartSec=.
specialist: S7
kind: satellite
...
## Cuándo usarla
Use when writing `/etc/systemd/system/X.service` unit with [Unit]/[Service]/[Install] sections, Type=, Restart=, RestartSec=.
