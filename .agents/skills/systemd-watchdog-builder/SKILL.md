---
name: systemd-watchdog-builder
description: Use when configuring `WatchdogSec=N` + sd_notify(WATCHDOG=1) in daemon code for systemd-driven liveness.
specialist: S7
kind: satellite
parent: stream-watchdog-sre
---

# systemd-watchdog-builder

## Cuándo usarla
Use when configuring `WatchdogSec=N` + sd_notify(WATCHDOG=1) in daemon code for systemd-driven liveness.
