---
name: tcp-initcwnd-tuner
description: Use when tuning initcwnd (initial congestion window) via `ip route change ... initcwnd 100` — current doctrine: 400 packets for IPTV high-BDP paths.
specialist: S8
kind: satellite
...
## Cuándo usarla
Use when tuning initcwnd (initial congestion window) via `ip route change ... initcwnd 100` — current doctrine: 400 packets for IPTV high-BDP paths.
