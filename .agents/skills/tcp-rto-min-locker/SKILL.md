---
name: tcp-rto-min-locker
description: Use when locking rto_min=40ms via `ip route change ... rto_min 40ms` for low-RTT routes to prevent slow retransmit.
specialist: S8
kind: satellite
...
## Cuándo usarla
Use when locking rto_min=40ms via `ip route change ... rto_min 40ms` for low-RTT routes to prevent slow retransmit.
