---
name: tls-1-2-fallback
description: Use when keeping TLS 1.2 as fallback `ssl_protocols TLSv1.2 TLSv1.3;` for legacy player compat (Android <7).
specialist: S8
kind: satellite
...
## Cuándo usarla
Use when keeping TLS 1.2 as fallback `ssl_protocols TLSv1.2 TLSv1.3;` for legacy player compat (Android <7).
