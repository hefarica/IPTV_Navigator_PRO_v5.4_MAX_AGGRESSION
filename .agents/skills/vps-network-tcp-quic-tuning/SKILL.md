---
name: vps-network-tcp-quic-tuning
description: TCP BBR, initcwnd/initrwnd, rto_min, MTU per WG tunnel (1340/1420), conntrack flush patterns, HTTP/2 vs HTTP/3 considerations for streaming, sub-second failover.
specialist: S8
kind: anchor
---

# vps-network-tcp-quic-tuning

## 1. Nombre de la skill
vps-network-tcp-quic-tuning

## 2. Cuándo usarla
TCP BBR, initcwnd/initrwnd, rto_min, MTU per WG tunnel (1340/1420), conntrack flush patterns, HTTP/2 vs HTTP/3 considerations for streaming, sub-second failover.

Anchored to specialist: **Network/TCP/QUIC Engineer** (S-tag S8).
Mission: TCP BBR, initcwnd 400, MTU per tunnel, sub-second WG failover
