---
name: x-frame-options-emitter
description: Use when emitting `X-Frame-Options: SAMEORIGIN` (or DENY) to prevent clickjacking on UI.
specialist: S10
kind: satellite
...
## Cuándo usarla
Use when emitting `X-Frame-Options: SAMEORIGIN` (or DENY) to prevent clickjacking on UI.
