---
name: x-xss-protection-emitter
description: Use when emitting `X-XSS-Protection: 1; mode=block` for legacy browser XSS filter.
specialist: S10
kind: satellite
...
## Cuándo usarla
Use when emitting `X-XSS-Protection: 1; mode=block` for legacy browser XSS filter.
