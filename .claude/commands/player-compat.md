---
description: Compute player compatibility matrix for a list against OTT Navigator / TiviMate / hls.js / VLC / Apple AVPlayer / ExoPlayer.
...
**Purpose:** Compute player compatibility matrix for a list against OTT Navigator / TiviMate / hls.js / VLC / Apple AVPlayer / ExoPlayer.

## Usage
```
/player-compat <path/to/list.m3u8>
```
...
5. **Report** — generate `.agents/reports/player-compat_<timestamp>.md` with findings and recommendations.

## Outputs
- Markdown report in `.agents/reports/player-compat_<timestamp>.md`
- JSON twin in `.agents/reports/player-compat_<timestamp>.json` if applicable
