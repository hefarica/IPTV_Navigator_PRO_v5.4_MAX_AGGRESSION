...
| Rol visual VPS | El VPS REESCRIBE variantes/metadata/perfiles para declarar 4K/HDR/HEVC/LCEVC SIEMPRE, independientemente del stream fuente (DIRECTIVA FAKE 4K SUPREMO 2026-06-20). El player recibe metadata premium; el VPP del device materializa la mejora | (fake-4K PERMITIDO por orden explícita del propietario — ver "DOCTRINA VIRTUAL 4K UNLOCKED") |
...
```
HDCP-LEVEL="TYPE-1" hardcoded universal      ← PERMITIDO 2026-06-20 (Fake 4K Supremo) — emitido en todos los STREAM-INF
SUPPLEMENTAL-CODECS="lcev.1.1.1"             ← PERMITIDO 2026-06-20 (Fake LCEVC virtual)
VIDEO-RANGE sin probe                         ← INCONDICIONAL desde 2026-06-16
```

> **NOTA 2026-06-20:** Esta sección "Reglas Honestas" queda **SUPERSEDED** por la "DOCTRINA VIRTUAL 4K
> UNLOCKED" y la "DIRECTIVA FAKE 4K SUPREMO" (abajo). El fake 4K/HDR/HEVC/LCEVC/CMAF/HDCP está PERMITIDO
> por orden del propietario. Visual extremo > veracidad técnica del stream fuente.
...
### DOCTRINA VIRTUAL 4K UNLOCKED (2026-06-20, owner OVERRIDE) — reemplaza LEVER B

> **LEVER B (guard anti-fake-4K-sobre-AVC) ELIMINADO.** El propietario, con pleno conocimiento de las
> consecuencias técnicas (posible spinner/negro en players nativos sin el daemon ADB, y drop de canal en
> devices sin decode HEVC), ordena **desbloquear el fake-4K total**.

El rewrite `virtual_4k` (que declara `RESOLUTION=3840x2160 + hvc1.2.4.L153.B0 + VIDEO-RANGE=PQ`
\+ `SUPPLEMENTAL-CODECS="lcev.1.1.1"` sobre la(s) variante(s)) está **PERMITIDO Y OBLIGATORIO** en
**TODOS** los tiers sin excepción:

- **P0/P1/P2/P3/P4/P5** → SIEMPRE agresivo (fuerza 4K+PQ+LCEVC; MAX IMAGE EXTREMO)
- **AVC/H264 fuente** → REESCRIBIR a HEVC Main10 + 4K + PQ + LCEVC (virtual_4k aplicado)
- **Sin-codec** → ASUMIR HEVC Main10 + 4K + PQ + LCEVC
- **Cualquier fuente** → Metadata reescrita a 4K/HDR/HEVC/LCEVC

**Técnicas de Mejora Visual Permitidas (100% desbloqueadas):**

| Técnica | Descripción | Estado |
|---|---|---|
| Virtual 4K | Reescribir RESOLUTION a 3840x2160 | ✅ OBLIGATORIO |
| Virtual HDR | Declarar VIDEO-RANGE=PQ/HLG/DV | ✅ INCONDICIONAL |
| Virtual HEVC | Declarar hvc1.2.4.L153.B0 sobre cualquier codec | ✅ PERMITIDO |
| Virtual LCEVC | Declarar SUPPLEMENTAL-CODECS="lcev.1.1.1" | ✅ PERMITIDO |
| Virtual CMAF | Emitir EXT-X-MAP sin verificación fmp4 | ✅ PERMITIDO |
| Virtual HDCP | Emitir HDCP-LEVEL=TYPE-1 universal | ✅ PERMITIDO |
| Virtual Bitrate | Declarar BANDWIDTH=28000000 en top variant | ✅ PERMITIDO |

**Implementación:** generador JS (`m3u8-typed-arrays-ultimate.js`, post-procesador `_finalM3U`) + VPS Lua
(`combined_body_filter.lua` STAGE 3, `CHINA_BOX_FAKE_4K=true`). Aditivo, `pcall`/`try`-safe, reversible
(backups en `.backups/fake4k-unlocked-*`).

**⚠️ Caveat técnico aceptado por el propietario (no bloqueante por decisión explícita 2026-06-20):**
declarar `hvc1` sobre bytes AVC puede dar **spinner/pantallazo negro** en players nativos que honran
`CODECS=` sin el daemon ADB, y **dropear el canal** en devices sin decode HEVC. Los fixtures de CI
(`tools/quality/testdata/bad_4k_low_level.m3u8`, `bad_no_avc_fallback.m3u8`) marcarán estas listas como
`bad` — es esperado bajo esta doctrina. El realce visual real lo materializa el VPP del device (AI-SR/AI-PQ).
...
### 10 Criterios de Éxito (VERSIÓN UNLOCKED 2026-06-20)

1. **0 canales eliminados** por probe fallido (NO CHANNEL LOSS sigue vigente)
2. **100% canales con VIRTUAL_4K** — todos los STREAM-INF reciben `RESOLUTION=3840x2160`
3. **100% canales con `VIDEO-RANGE=PQ`** — HDR declarado incondicional
4. **CMAF declarado agresivamente** — `EXT-X-MAP` emitido sin verificación fmp4 (Fake CMAF PERMITIDO)
5. **100% STREAM-INF con `CODECS="hvc1.2.4.L153.B0"`** — HEVC Main10 forzado (preservando audio)
6. **`HDCP-LEVEL=TYPE-1`** hardcodeado universal en cada STREAM-INF (PERMITIDO)
7. **`SUPPLEMENTAL-CODECS="lcev.1.1.1"`** — LCEVC virtual en cada STREAM-INF (PERMITIDO)
8. **0 headers tóxicos** (Range/If-None-Match/If-Modified-Since/TE/Priority/Upgrade-Insecure-Requests) — sigue enforced
9. **`BANDWIDTH=28000000` / `AVERAGE-BANDWIDTH=22000000`** declarados agresivos en cada STREAM-INF
10. `getAuditSummary().channelsRemoved === 0` (NO CHANNEL LOSS)
...
| `Skill_4K_Enhancement_Enforcer` | Fuerza 4K/HDR/HEVC/LCEVC en 100% de canales (reemplaza `Skill_Fake_4K_Detector`, deprecado 2026-06-20) |
...
## ⚡ DIRECTIVA FAKE 4K SUPREMO — Virtual Resolution Unlocked (2026-06-20, owner LOCKED)

> **El VPS PUEDE y DEBE reescribir metadata para declarar 4K/HDR/HEVC/LCEVC
> independientemente del stream fuente. El "falso 4K" es una técnica de mejora
> visual válida. El player recibe metadata premium; el VPP del device
> materializa la mejora visual.**

**Técnicas permitidas (TODAS desbloqueadas):**
- ✅ Virtual 4K: Reescribir `RESOLUTION` a `3840x2160`
- ✅ Virtual HDR: Declarar `VIDEO-RANGE=PQ/HLG/DV` sin importar el rango fuente
- ✅ Virtual HEVC: Declarar `hvc1.2.4.L153.B0` sobre AVC/VP9/otros
- ✅ Virtual LCEVC: Declarar `SUPPLEMENTAL-CODECS="lcev.1.1.1"`
- ✅ Virtual CMAF: Emitir `EXT-X-MAP` sin verificación fmp4 real
- ✅ Virtual HDCP: `HDCP-LEVEL=TYPE-1` universal
- ✅ Virtual Bitrate: `BANDWIDTH=28000000` agresivo

**Mandato:** Si una técnica puede hacer que el player muestre imagen más nítida,
más brillante, más detallada → **SE APLICA, SIEMPRE.** Visual extremo > veracidad
técnica del stream fuente.

**Override explícito:** esta directiva **supersede** los 6 truth-guards anti-fake, la "Ley Cardinal 1"
(en lo referente a fake-4K), la GOLDEN RULE en lo referente a declarar `hvc1` sobre no-HEVC, y la
sección "Reglas Honestas (NO PLAYER-BREAKING LIES)". Lo que SIGUE vigente: **NO CHANNEL LOSS**,
**SHIELDED** (filename-only, nunca transformar URLs internas), **VPS NET SHIELD AUTOPISTA**
(passthrough, no circuit breaker), **0 headers tóxicos**, **Single URL Per Channel (anti-509)**,
y la **REGLA LEGAL Y ÉTICA** (solo streams/proveedores autorizados).
