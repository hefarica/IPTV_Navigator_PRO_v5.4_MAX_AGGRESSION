...
# Cadencias. FREEZE-SAFE: la prioridad en ACTIVE_PLAYER_MODE es NO interrumpir
# la reproduccion. Loops y sondeos se relajan mientras hay player en foreground.
WATCH_INTERVAL=20
ACTIVE_INTERVAL=5            # freeze-safe: antes 1s (loop demasiado agresivo en ONN 4K)
PQ_INTERVAL=300
PQ_ACTIVE_INTERVAL=600       # freeze-safe: re-enforce de imagen diferido en playback
BASELINE_INTERVAL=60
MANIFEST_WATCH_INTERVAL=60   # freeze-safe: check de hash remoto cada 60s (antes 20)
MANIFEST_ACTIVE_INTERVAL=60  # freeze-safe: check hash cada 60s en playback (antes 5)
METRIC_INTERVAL=15           # freeze-safe: ping activo condicionado, max cada 15s (antes 3)
HEARTBEAT_WATCH_INTERVAL=60
HEARTBEAT_ACTIVE_INTERVAL=30 # freeze-safe: heartbeat cada 30s en playback (antes 15)

# RAM.
SOFT_LIMIT_MB="${SOFT_LIMIT_MB:-200}"
HARD_LIMIT_MB="${HARD_LIMIT_MB:-100}"
# FREEZE-SAFE: durante playback el cleanup solo dispara con umbrales mas estrictos
# y solo tras N muestras consecutivas, para no robar CPU/IO al decoder.
ACTIVE_SOFT_LIMIT_MB="${ACTIVE_SOFT_LIMIT_MB:-120}"
ACTIVE_HARD_LIMIT_MB="${ACTIVE_HARD_LIMIT_MB:-60}"
MEM_LOW_SAMPLES_NEEDED=3
MEM_LOW_STREAK=0
MAX_LOG_LINES=700

# Red / emergencia. FREEZE-SAFE: VPN restart con histeresis (no por 1 solo fallo).
VPN_RESTART_COOLDOWN=180     # freeze-safe: antes 120s
LAST_VPN_RESTART=0
LAST_VPS_TRIGGER=0
...
RX_PREV=""
TS_PREV=""
# FREEZE-SAFE: contadores de histeresis para decidir restart de VPN.
TUN0_DOWN_STREAK=0
LOSS_FAIL_STREAK=0
RX_STALL_STREAK=0
LAST_RX_FOR_STALL=""
...
soft_cleanup() {
    ...
    for pkg in $KILL_TARGETS; do
        # FREEZE-SAFE (defensa en profundidad): jamas matar un paquete protegido,
        # aunque por error apareciera en KILL_TARGETS. Player/VPN/UI nunca caen.
...
restart_vpn_if_needed() {
    # FREEZE-SAFE: histeresis. NO reiniciar la VPN por un unico fallo (un restart
    # mid-stream mete corte garantizado). Se exige confirmacion sostenida:
    #   - tun0 abajo en >=2 ciclos consecutivos, O
    #   - >=2 de las 3 senales (tun0 down, packet loss alto, rx_bytes congelado).
    if ip addr show tun0 >/dev/null 2>&1; then
        TUN0_DOWN_STREAK=0
        return 0
    fi
    TUN0_DOWN_STREAK=$((TUN0_DOWN_STREAK + 1))

    # Senal de rx congelado (sin transferencia entrante).
    rx_now=$(get_rx_bytes)
    if [ -n "$LAST_RX_FOR_STALL" ] && [ "$rx_now" = "$LAST_RX_FOR_STALL" ]; then
        RX_STALL_STREAK=$((RX_STALL_STREAK + 1))
    else
        RX_STALL_STREAK=0
    fi
    LAST_RX_FOR_STALL="$rx_now"

    # Conteo de senales activas (2-de-3).
    signals=0
    [ "$TUN0_DOWN_STREAK" -ge 1 ] 2>/dev/null && signals=$((signals + 1))
    [ "$PACKET_LOSS" -ge 80 ] 2>/dev/null && signals=$((signals + 1))
    [ "$RX_STALL_STREAK" -ge 3 ] 2>/dev/null && signals=$((signals + 1))

    # Condicion de restart: tun0 down confirmado 2 ciclos, O 2-de-3 senales.
    if [ "$TUN0_DOWN_STREAK" -lt 2 ] 2>/dev/null && [ "$signals" -lt 2 ] 2>/dev/null; then
        return 1
    fi
    ...
    log "VPN: hysteresis trip tun0_streak=$TUN0_DOWN_STREAK signals=$signals loss=$PACKET_LOSS rx_stall=$RX_STALL_STREAK"
    TUN0_DOWN_STREAK=0
    RX_STALL_STREAK=0
...
update_network_metrics() {
    # FREEZE-SAFE: ping ligero -c 1 -W 1 (no -c 3). El llamador ya condiciona la
    # frecuencia (METRIC_INTERVAL=15s) y solo cuando el throughput pasivo lo amerita.
    ping_res=$(ping -c 1 -W 1 "$VPS_IP" 2>&1)
...
daemon_loop() {
    ...
    while true; do
        ...
        # FREEZE-SAFE: re-enforce periodico de PQ/manifest. En WATCH_MODE cada
        # PQ_INTERVAL (300s); en ACTIVE_PLAYER_MODE se difiere a 600s para no
        # reescribir settings de imagen durante reproduccion. Los triggers
        # explicitos (archivo trigger / SIGUSR1) arriba siguen aplicando al instante.
        pq_interval_eff="$PQ_INTERVAL"
        [ "$CURRENT_MODE" = "ACTIVE_PLAYER_MODE" ] && pq_interval_eff="$PQ_ACTIVE_INTERVAL"
        ...
        mem=$(get_mem_mb MemAvailable)
        if [ "$CURRENT_MODE" = "ACTIVE_PLAYER_MODE" ]; then
            # FREEZE-SAFE: durante playback, umbrales estrictos + N muestras
            # consecutivas. hard_cleanup (drop_caches/compact_memory) BLOQUEADO
            # salvo emergencia extrema sostenida, para no robar IO al decoder.
            if [ "$mem" -lt "$ACTIVE_SOFT_LIMIT_MB" ] 2>/dev/null; then
                MEM_LOW_STREAK=$((MEM_LOW_STREAK + 1))
            else
                MEM_LOW_STREAK=0
            fi
            if [ "$MEM_LOW_STREAK" -ge "$MEM_LOW_SAMPLES_NEEDED" ] 2>/dev/null; then
                if [ "$mem" -lt "$ACTIVE_HARD_LIMIT_MB" ] 2>/dev/null; then
                    log "RAM_HARD_ACTIVE_EMERGENCY: ${mem}MB streak=$MEM_LOW_STREAK"
                    hard_cleanup
                else
                    soft_cleanup
                fi
                ...
                [ "$mem" -lt "$ACTIVE_HARD_LIMIT_MB" ] 2>/dev/null && \
                    log "RAM_HARD_SKIPPED_ACTIVE_PLAYER: ${mem}MB streak=$MEM_LOW_STREAK (protecting playback)"
            fi
        else
            MEM_LOW_STREAK=0
...
# health-json: imprime JSON valido en stdout para que el watchdog del VPS lo
# recoja por ADB (prompt#3 componente 1.R). NO depende del heartbeat HTTP.
health_json() {
    pid=""; alive=false
    if [ -f "$LOCK" ]; then
        pid=$(cat "$LOCK" 2>/dev/null | tr -dc '0-9')
        is_alive_pid "$pid" && alive=true
    fi
    mem=$(get_mem_mb MemAvailable)
    vpn="DOWN"; ip addr show tun0 >/dev/null 2>&1 && vpn="UP"
    player="OFF"; is_player_foreground && player="FOREGROUND"
    ...
    printf '{"daemon":"ape-uhdx-sentinel","device":"%s","pid":"%s","alive":%s,"profile":"%s","ram_avail_mb":%s,"tun0":"%s","player":"%s","ai_sr_level":"%s","hdr_conversion_mode":"%s","manifest_hash":"%s","uptime":%s,"ts":"%s"}\n' \
        "$device" "$pid" "$alive" "$profile" "${mem:-0}" "$vpn" "$player" "$ai_sr" "$hdr_mode" "$mhash" "${uptime:-0}" "$ts"
}
...
case "${1:-daemon}" in
    ...
    selftest)
        ...
        ;;
    *)
        echo "Usage: $0 {daemon|install|apply|manifest-now|manifest-apply|health-json|profile-sdr|profile-hdr|profile-auto|cleanup|status|stop|selftest}"
        exit 1
        ;;
esac
