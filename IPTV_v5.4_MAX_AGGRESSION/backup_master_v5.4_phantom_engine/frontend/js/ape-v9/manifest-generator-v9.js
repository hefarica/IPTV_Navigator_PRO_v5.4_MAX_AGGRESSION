...
class ManifestGeneratorEnterprise {
    constructor(config = {}) {
        this.config = {
            ...
            jwtExpiry: 31536000, // 365 días (1 año completo)
...
`;
    ...
    _generateEnterpriseChannelEntry(channel, options = {}) {
        ...
        entry += `#EXT-X-STREAM-INF:BANDWIDTH=5000000,AVERAGE-BANDWIDTH=4000000,RESOLUTION=1920x1080,CODECS="avc1.64001f,mp4a.40.2"\n`;
...
`;
        ...
        for (const quality of qualities) {
            master += `#EXT-X-STREAM-INF:BANDWIDTH=${quality.bandwidth},AVERAGE-BANDWIDTH=${Math.round(quality.bandwidth * 0.8)},RESOLUTION=${quality.resolution},FRAME-RATE=${quality.fps},CODECS="${quality.codec},mp4a.40.2",HDR-PROFILE=HLG\n`;
...
`;
    ...
    _generateHeaders(channel) {
        ...
        if (window.userAgentRotation) {
            selectedUA = window.userAgentRotation.selectRandomUserAgent();
        } else {
            const userAgents = [
        ...
        return {
            ...
            'range': 'bytes=0-',
            // REMOVED: x-forwarded-for, x-real-ip - these trigger CDN blocks
