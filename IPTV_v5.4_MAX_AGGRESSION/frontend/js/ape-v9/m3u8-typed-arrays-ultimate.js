...
#EXT-X-APE-SYNC-SLAVE:MASTER_ORIGIN_SYNC_V2`;
    ...
    function generateChannelEntry(channel, profile = 'P3', index = 0, credentialsMap = {}, options = {}) {
        ...
        // ═══ VIRTUAL 4K UNLOCKED (2026-06-20, owner LOCKED) — Fake 4K/HDR/HEVC/LCEVC/HDCP ═══
        // DIRECTIVA FAKE 4K SUPREMO: reescribe metadata premium en CADA #EXT-X-STREAM-INF del bloque,
        // sin importar codec/resolución/rango fuente. Aplica a 100% de canales (P0-P5). Reemplaza LEVER B.
        // CAVEAT (aceptado por el propietario): declarar hvc1 sobre bytes AVC puede dar spinner/negro en
        // players nativos sin el daemon ADB, y dropear el canal en devices sin decode HEVC. El VPP del
        // device (AI-SR/AI-PQ) materializa la mejora visual. try-safe: nunca bloquea la generación.
        try {
            const V4K = {
                enabled: true,
                resolution: '3840x2160',
                ...
                videoRange: 'PQ',
                supplementalCodecs: 'lcev.1.1.1',
                bandwidth: 28000000,
                avgBandwidth: 22000000,
                hdcpLevel: 'TYPE-1'
            };
            if (V4K.enabled && _finalM3U.indexOf('#EXT-X-STREAM-INF:') !== -1) {
                _finalM3U = _finalM3U.replace(/#EXT-X-STREAM-INF:[^\r\n]*/g, (infLine) => {
                    let s = infLine;
                    // 1. RESOLUTION -> 4K
                    if (/RESOLUTION=\d+x\d+/.test(s)) {
                        s = s.replace(/RESOLUTION=\d+x\d+/, 'RESOLUTION=' + V4K.resolution);
                    } else {
                        s += ',RESOLUTION=' + V4K.resolution;
                    }
                    // 2. CODECS -> HEVC Main10 (preservando audio existente)
                    const am = s.match(/CODECS="([^"]*)"/);
                    if (am) {
                        const ac = am[1].split(',').map(x => x.trim())
                            .find(p => /^(mp4a|ac-3|ec-3|opus|dts)/i.test(p));
                        const newc = V4K.codecs + (ac ? ',' + ac : '');
                        s = s.replace(/CODECS="[^"]*"/, 'CODECS="' + newc + '"');
                    } else {
                        s += ',CODECS="' + V4K.codecs + '"';
                    }
                    // 3. SUPPLEMENTAL-CODECS -> LCEVC virtual
                    if (!/SUPPLEMENTAL-CODECS=/.test(s)) {
                        s += ',SUPPLEMENTAL-CODECS="' + V4K.supplementalCodecs + '"';
                    }
                    // 4. VIDEO-RANGE -> PQ
                    if (/VIDEO-RANGE=/.test(s)) {
                        s = s.replace(/VIDEO-RANGE=[A-Za-z]+/, 'VIDEO-RANGE=' + V4K.videoRange);
                    } else {
                        s += ',VIDEO-RANGE=' + V4K.videoRange;
                    }
                    // 5. BANDWIDTH agresivo (AVERAGE primero; BANDWIDTH plano anclado a ':'/',')
                    if (/AVERAGE-BANDWIDTH=\d+/.test(s)) {
                        s = s.replace(/AVERAGE-BANDWIDTH=\d+/, 'AVERAGE-BANDWIDTH=' + V4K.avgBandwidth);
                    }
                    if (/(^|[,:])BANDWIDTH=\d+/.test(s)) {
                        s = s.replace(/(^|[,:])BANDWIDTH=\d+/, '$1BANDWIDTH=' + V4K.bandwidth);
                    } else {
                        s += ',BANDWIDTH=' + V4K.bandwidth;
                    }
                    // 6. HDCP-LEVEL universal
                    if (/HDCP-LEVEL=/.test(s)) {
                        s = s.replace(/HDCP-LEVEL=[A-Za-z0-9-]+/, 'HDCP-LEVEL=' + V4K.hdcpLevel);
                    } else {
                        s += ',HDCP-LEVEL=' + V4K.hdcpLevel;
                    }
                    return s;
                });
                // Observability marker (RFC 8216 §6.3.1 tag privado, invisible al player)
                _finalM3U = _finalM3U.replace(/#EXT-X-STREAM-INF:/,
                    '#EXT-X-APE-VIRTUAL-4K:enabled=true,method=aggressive,tier=all,avc_rewrite=true\n#EXT-X-STREAM-INF:');
            }
        } catch (_) { /* never block generation on virtual-4k rewrite */ }
