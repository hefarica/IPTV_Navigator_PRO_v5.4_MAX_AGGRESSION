...
elseif status >= 500 and status <= 504 then
    -- Server error → retry corto las primeras 5 veces, después cooldown
    -- Esto previene loop infinito cuando provider tira 5xx sostenido
    -- (caso real 2026-04-26 canal 503226: retry escaló a 62 sin freno)
    if retry <= 5 then
        delay_ms = rand(500, 1500)
        dict:set(k_retry, retry)
    else
        -- Loop sostenido en 5xx → cooldown 30-60s para liberar provider slot
        delay_ms = rand(30 * 1000, 60 * 1000)
        dict:set(k_retry, 0)
    end
else
