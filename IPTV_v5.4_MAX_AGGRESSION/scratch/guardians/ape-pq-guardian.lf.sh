...
# ═══════════════════════════════════════════════════════════════════
enforce_display() {
    # [AFR FIX 2026-05-21] DO NOT pin 60Hz. Pinning 3840x2160@60 overrode AFR and
    # caused judder on 50/24/25fps content (60Hz vs 50fps = fractional cadence).
    # Instead: keep the preferred mode FREE + enable AFR (match_content_frame_rate=2)
    # so the display matches the content refresh (4K@50 for 50fps, 4K@24 for 24fps),
    # still 4K. The TV (Samsung 2017) supports 4K@60/50/30/25/24.
    local pref=$(cmd display get-user-preferred-display-mode 2>/dev/null)
    if echo "$pref" | grep -qi "null"; then
        log "  DISPLAY: preferred mode free (AFR matches content refresh) - OK"
    else
        cmd display clear-user-preferred-display-mode 2>/dev/null
        log "  DISPLAY: cleared pinned mode -> AFR controls refresh (anti-judder)"
    fi
    settings put secure match_content_frame_rate 2 2>/dev/null
    settings put global match_content_frame_rate 2 2>/dev/null
