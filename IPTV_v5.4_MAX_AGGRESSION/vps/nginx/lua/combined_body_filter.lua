...
-- ═══ STAGE 3: VIRTUAL 4K UNLOCKED (2026-06-20, owner LOCKED) ═══════════
-- Fake 4K/HDR/HEVC/LCEVC/CMAF/HDCP sobre TODAS las variantes, TODOS los tiers (P0-P5).
-- Reemplaza LEVER B (guard anti-fake-4K-sobre-AVC ELIMINADO). pcall-safe + autopista
-- (passthrough on error). Caveat aceptado por el propietario: declarar hvc1 sobre bytes
-- AVC puede dar spinner/negro en players nativos sin el daemon ADB, y dropear el canal
-- en devices sin decode HEVC. El realce real lo materializa el VPP del device (AI-SR/AI-PQ).
local v4k3_ok, v4k3_err = pcall(function()
    if ngx.status ~= 200 then return end
    local ct = ngx.header["Content-Type"] or ngx.header["content-type"] or ""
    local is_hls = ct:find("mpegurl", 1, true) or ct:find("mpegURL", 1, true) or ct:find("octet-stream", 1, true)
    if not is_hls then return end
    if not body:find("#EXT-X-STREAM-INF", 1, true) then return end

    local V4K_CODEC = "hvc1.2.4.L153.B0"
    local V4K_LCEVC = "lcev.1.1.1"
    local V4K_BW    = "28000000"
    local V4K_ABW   = "22000000"

    local out = {}
    local modified = false
    local has_map = body:find("#EXT-X-MAP", 1, true) ~= nil
    for line in body:gmatch("[^\r\n]+") do
        if line:find("^#EXT%-X%-STREAM%-INF:") then
            local s = line
            -- 1. RESOLUTION -> 4K
            if s:find("RESOLUTION=%d+x%d+") then
                s = s:gsub("RESOLUTION=%d+x%d+", "RESOLUTION=3840x2160")
            else
                s = s .. ",RESOLUTION=3840x2160"
            end
            -- 2. CODECS -> HEVC Main10 (preservar audio existente)
            local cur = s:match('CODECS="([^"]*)"')
            if cur then
                local audio = cur:match("mp4a%.[%w%.]+") or cur:match("ac%-3") or cur:match("ec%-3")
                local newc = V4K_CODEC
                if audio then newc = newc .. "," .. audio end
                s = s:gsub('CODECS="[^"]*"', 'CODECS="' .. newc .. '"')
            else
                s = s .. ',CODECS="' .. V4K_CODEC .. '"'
            end
            -- 3. SUPPLEMENTAL-CODECS -> LCEVC virtual
            if not s:find("SUPPLEMENTAL%-CODECS=") then
                s = s .. ',SUPPLEMENTAL-CODECS="' .. V4K_LCEVC .. '"'
            end
            -- 4. VIDEO-RANGE -> PQ
            if s:find("VIDEO%-RANGE=") then
                s = s:gsub("VIDEO%-RANGE=[%w]+", "VIDEO-RANGE=PQ")
            else
                s = s .. ",VIDEO-RANGE=PQ"
            end
            -- 5. BANDWIDTH agresivo (AVERAGE primero; BANDWIDTH plano anclado a ':'/',')
            if s:find("AVERAGE%-BANDWIDTH=%d+") then
                s = s:gsub("AVERAGE%-BANDWIDTH=%d+", "AVERAGE-BANDWIDTH=" .. V4K_ABW)
            end
            if s:find("[:,]BANDWIDTH=%d+") then
                s = s:gsub("([:,])BANDWIDTH=%d+", "%1BANDWIDTH=" .. V4K_BW)
            else
                s = s .. ",BANDWIDTH=" .. V4K_BW
            end
            -- 6. HDCP-LEVEL universal
            if s:find("HDCP%-LEVEL=") then
                s = s:gsub("HDCP%-LEVEL=[%w%-]+", "HDCP-LEVEL=TYPE-1")
            else
                s = s .. ",HDCP-LEVEL=TYPE-1"
            end
            out[#out + 1] = s
            modified = true
        else
    ...
    if modified then
        -- 7. CMAF virtual: EXT-X-MAP si no existe (master playlist)
        if not has_map then
            local base = (ngx.var.uri or "/"):match("(.*/)") or "/"
            local final = {}
            local injected = false
            for _, l in ipairs(out) do
                if (not injected) and l:find("^#EXTM3U") then
                    final[#final + 1] = l
                    final[#final + 1] = '#EXT-X-MAP:URI="' .. base .. 'init.mp4"'
                    injected = true
                else
                    final[#final + 1] = l
                end
            end
            out = injected and final or out
        end
        body = table.concat(out, "\n") .. "\n"
        pcall(function()
            ngx.header["X-APE-Virtual-4K"] = "enabled=aggressive;tiers=all;avc_rewrite=true;hdr=PQ;hevc=Main10;lcevc=1.1.1;hdcp=TYPE-1"
        end)
        local R = ngx.shared.circuit_metrics
        if R then pcall(function() R:set("v4k_applied", 1) end) end
        ngx.log(ngx.NOTICE, "[VIRTUAL-4K-UNLOCKED] aggressive 4K/HDR/HEVC/LCEVC/HDCP rewrite applied")
    end
end) -- pcall virtual_4k_unlocked
if not v4k3_ok then
    ngx.log(ngx.WARN, "VIRTUAL_4K_UNLOCKED_ERR: " .. tostring(v4k3_err))
end
