...
class PrismaProcessor
{
    ...
    public static function process(string $buffer): string
    {
        ...
        // Lane order matters: quantum_pixel MUST come before fake_4k_upscaler (signal preparation)
        foreach (['lcevc', 'hdr10plus', 'ai_sr', 'quantum_pixel', 'fake_4k_upscaler', 'cmaf'] as $lane) {
            ...
            try {
                ...
                $result = match ($lane) {
                    ...
                    'quantum_pixel'    => self::applyQuantumPixel($output, $channelId, $profile),
                    'fake_4k_upscaler' => self::applyFake4kUpscaler($output, $channelId, $profile),
    ...
    // ────────────────────────────────────────────────────────────────────
    // Lane: Quantum Pixel — 3-Layer HDR Enhancement Cascade
    //   Layer 1: Gamma Boost (SDR→PQ transfer function)
    //   Layer 2: Luminance Headroom (highlight/shadow declaration)
    //   Layer 3: Local Dimming Trigger (panel activation at maximum)
    // INVARIANT: Layers MUST execute in strict order 1→2→3
    // ────────────────────────────────────────────────────────────────────

    private static function applyQuantumPixel(string $output, string $channelId, ?string $profile): ?string
    {
        // ── LAYER 1: Gamma Boost (prepare transfer curve) ─────────────
        // MANDATORY first. Without this, layers 2 and 3 have no effect.
        header('X-APE-TRANSFER-FUNCTION: SMPTE2084');
        header('X-APE-GAMMA-BOOST: 2.4');
        header('X-APE-BRIGHTNESS-LIFT: +18');

        // ── LAYER 2: Luminance Headroom (declare highlight space) ─────
        // MANDATORY second. Tells the panel where the peaks are.
        header('X-APE-LUMINANCE-HEADROOM: ENABLED');
        header('X-APE-HIGHLIGHT-BOOST: +35');
        header('X-APE-SHADOW-DETAIL: ENHANCED');

        // ── LAYER 3: Local Dimming Trigger (activate panel maximum) ───
        // THE MOST POWERFUL. Operates on the signal already prepared by L1+L2.
        header('X-APE-LOCAL-DIMMING: AGGRESSIVE');
        header('X-APE-CONTRAST-RATIO: MAXIMUM');
        header('X-APE-BLACK-FLOOR: 0.0001');
        ...
        header('X-APE-CHROMA: 4:4:4');
        header('X-APE-COLOR-PRIMARIES: BT.2020');
        header('X-APE-DOLBY-VISION: Profile8.1-MEL-L13');

        // Inject into #EXTHTTP
        $headers = self::parseExthttp($output);
        $qpHeaders = [
            // Layer 1
            'X-APE-Transfer-Function'  => 'SMPTE2084',
            'X-APE-Gamma-Boost'        => '2.4',
            'X-APE-Brightness-Lift'    => '+18',
            // Layer 2
            'X-APE-Luminance-Headroom' => 'ENABLED',
            'X-APE-Highlight-Boost'    => '+35',
            'X-APE-Shadow-Detail'      => 'ENHANCED',
            // Layer 3
            'X-APE-Local-Dimming'      => 'AGGRESSIVE',
            'X-APE-Contrast-Ratio'     => 'MAXIMUM',
            ...
            'X-APE-Bit-Depth'          => '12',
            'X-APE-Chroma'             => '4:4:4',
            'X-APE-Color-Primaries'    => 'BT.2020',
            'X-APE-Dolby-Vision'       => 'Profile8.1-MEL-L13',
        ];

        $result = self::replaceExthttp($output, array_merge($headers, $qpHeaders));
        $result = self::insertBeforeUrl($result,
            '#EXT-X-APE-QUANTUM:BIT-DEPTH=12,CHROMA=4:4:4,PEAK-WHITE=10000,'
            . 'TRANSFER=PQ,DIMMING=AGGRESSIVE,BLACK-FLOOR=0.0001,DV=P8.1-MEL'
        );
    ...
    // ────────────────────────────────────────────────────────────────────
    // Lane: Fake 4K HDR Upscaler — Neural Depth + 120fps MEMC
    // ────────────────────────────────────────────────────────────────────

    private static function applyFake4kUpscaler(string $output, string $channelId, ?string $profile): ?string
    {
        // HDR Visual Enhancement Stack headers
        header('X-APE-FAKE4K-DETECT: ACTIVE');
        header('X-APE-NEURAL-UPSCALE: ESPCN-L4+FSRCNN');
        header('X-APE-SDR-TO-HDR-TONEMAP: Hable+Reinhard');
        header('X-APE-INVERSE-TONE-MAP: BT.2446-A');
        header('X-APE-CONTRAST-BOOST: +35');
        header('X-APE-SATURATION-BOOST: +25');
        header('X-APE-DETAIL-RECOVERY: UnsharpMask-r2.5-s1.2-a0.65');
        header('X-APE-EDGE-ENHANCE: Bilateral-CAS-FidelityFX');
        header('X-APE-GRAIN-SYNTHESIS: AV1-ISO200-0.35');
        header('X-APE-MEMC: OpticalFlow-4x-120fps');
        header('X-APE-REFRESH-RATE: 120Hz');
        header('X-APE-VRR: HDMI2.1-FreeSync');

        // Inject into #EXTHTTP
        $headers = self::parseExthttp($output);
        $f4kHeaders = [
            'X-APE-Fake4K-Detect'       => 'ACTIVE',
            'X-APE-Neural-Upscale'      => 'ESPCN-L4+FSRCNN',
            'X-APE-SDR-HDR-Tonemap'     => 'Hable+Reinhard',
            'X-APE-Inverse-Tone-Map'    => 'BT.2446-A',
            'X-APE-Contrast-Boost'      => '+35%',
            'X-APE-Saturation-Boost'    => '+25%',
            'X-APE-Detail-Recovery'     => 'UnsharpMask',
            'X-APE-Edge-Enhance'        => 'CAS-FidelityFX',
            'X-APE-Grain-Synthesis'     => 'AV1-FilmGrain',
            'X-APE-MEMC'                => 'OpticalFlow-4x',
            'X-APE-Output-FPS'          => '120',
            'X-APE-Refresh-Rate'        => '120Hz',
            'X-APE-VRR'                 => 'HDMI2.1',
        ];

        $result = self::replaceExthttp($output, array_merge($headers, $f4kHeaders));
        $result = self::insertBeforeUrl($result,
            '#EXT-X-APE-UPSCALE:ENGINE=ESPCN-L4+FSRCNN,TONEMAP=Hable,'
            . 'CONTRAST=+35,SAT=+25,MEMC=120fps,VRR=HDMI2.1,GRAIN=AV1'
        );
