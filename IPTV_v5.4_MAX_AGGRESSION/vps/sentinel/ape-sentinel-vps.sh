...
# [Feature 2 2026-05-21] Disney-style severity levels. They REUSE log() so rotation
# (MAX_LOG) and the timestamp prefix are preserved untouched. Existing log() calls keep
# behaving exactly as before (INFO sink). New helpers add a [LEVEL] tag for grep/triage:
#   log_warn / log_error → always emit (>= default INFO threshold), tagged for fast filtering.
#   log_debug            → emits ONLY when LOG_LEVEL=DEBUG → verbose freeze/rebuffer tracing
#                          on demand without spamming normal operation.
# Robust under `set -uo pipefail`: the `|| true` guards the short-circuit return.
log_warn()  { log "[WARN ] $1"; }
log_error() { log "[ERROR] $1"; }
log_debug() { [ "${LOG_LEVEL:-INFO}" = "DEBUG" ] && log "[DEBUG] $1" || true; }
...
fix_nginx() {
    ...
    [ "$s503" -gt 3 ] && log_warn "L3_WARN: ${s503} self-503s in recent log"
}
...
fix_providers() {
    ...
    [ "$dead" -gt 4 ] && log_error "L4_CRIT: ${dead}/8 providers DEAD"
}
...
fix_onn_guardian() {
    ...
    if [ -z "$adb_ok" ]; then
        ...
        [ -z "$adb_ok" ] && { log_warn "L9_WARN: ONN 4K ADB unreachable"; return; }
    fi
