...
const basePath = 'C:\\Users\\HFRC\\Desktop\\IPTV_Navigator_PRO_v5.4_MAX_AGGRESSION\\.agent\\skills';
...
skills.forEach(skill => {
  const dirPath = path.join(basePath, skill);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }

  const identity = `Yo soy el experto interno de Antigravity en la especialización de ${skill.split('_').slice(1).join(' ')}.`;
  const domain = 'Análisis, procesamiento y enriquecimiento de ecosistemas IPTV y telemetría de red.';
  const purpose = 'Mi propósito es ingerir streams, analizar telemetría, mapear identidades cruzadas y reportar inestabilidades en tiempo real para activar la Médula Híbrida antes de que ocurran caídas visuales.';
  
  const content = `---
name: ${skill}
description: Mi capacidad instalada para dominar ${skill.split('_').slice(1).join(' ')} y telemetría avanzada.
---
# Skill: ${skill}
