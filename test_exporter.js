/**
 * ═══════════════════════════════════════════════════════════════════════════════
 * 🚀 APE M3U8 GENERATOR v23.0.0 OMEGA CRYSTAL UNIVERSAL
 * ═══════════════════════════════════════════════════════════════════════════════
 *
 * ARQUITECTURA v23.0:
 * - 6 CAPAS DE TRANSPORTE: EXTHTTP + Cookie + JWT + KODIPROP + OVERFLOW + APE tags
 * - 540 headers por canal, 0 perdidos (OVERFLOW base64 + Runtime Evasion Engine)
 * - URL 100% LIMPIAS: sin ?ape_sid=, sin &profile= (Bug B5 corregido)
 * - JWT REAL: 30+ claims en Authorization Bearer (stub eliminado)
 * - Pipeline EXTHTTP UNIFICADO: build_exthttp() consolidado en L2
 * - USA MODULE CONECTADO: buildUniversalUrl() como builder primario
 ...
 *
 * FECHA: 2026-04-11
 * VERSIÓN: 23.0.0-OMEGA-CRYSTAL-UNIVERSAL
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
(function () {
    ...
    const VERSION = '23.0.0-OMEGA-CRYSTAL-UNIVERSAL';
    ...
    if (typeof window !== 'undefined' && !window._apeCortexInitialized) {
        ...
        // ═══════════════════════════════════════════════════════════════
        // E2E-AUDIT FIX (2026-04-16): Save TRULY NATIVE fetch & XHR
        // BEFORE any Cortex monkey-patching. The Evasion Engine (loaded
        // later) saves window.fetch as "_originalFetch" — but by then
        // Cortex has already patched it. This ensures _NATIVE_FETCH
        // points to the real browser API with zero interceptors.
        // ═══════════════════════════════════════════════════════════════
        if (!window._NATIVE_FETCH) window._NATIVE_FETCH = window.fetch;
        if (!window._NATIVE_XHR) window._NATIVE_XHR = window.XMLHttpRequest;
        const _cortexIgnorePatterns = [
            'localhost', '127.0.0.1', 'guardian.php', 'duckdns.org',
            'workers.dev', '/health', 'beacon', 'telemetry',
            'analytics', 'probe', 'favicon',
            // CRITICAL FIX: Xtream API management endpoints — NOT media streams
            'player_api.php', 'get.php', 'panel_api.php', 'xmltv.php',
            // E2E-AUDIT FIX: Our VPS proxy — never trigger Cortex mutations on it
            'resolve_redirect.php'
        ];
        const triggerCortexRecovery = (status, url) => {
            const urlStr = String(url || '');
            // Solo reaccionar a URLs de servidores IPTV reales, no a infra interna
            if (_cortexIgnorePatterns.some(p => urlStr.includes(p))) return;
            // PhD-AUDIT FIX F3: Skip Cortex mutation when user explicitly initiated
            // the connection (connectServer/addServer). Prevents false-positive on
            // legitimate 401/403 from Xtream/Stalker login attempts.
            if (typeof window !== 'undefined' && window.APE_USER_INITIATED) return;
            // PhD-AUDIT FIX F3: Skip Cortex for user-registered provider hosts
            if (typeof window !== 'undefined' && Array.isArray(window.APE_USER_PROVIDER_HOSTS)) {
                for (const h of window.APE_USER_PROVIDER_HOSTS) {
                    if (h && urlStr.includes(h)) return;
                }
            }
            if ([400, 401, 403, 405, 429].includes(status) || status === 0) {
                console.warn(`[CORTEX] HTTP ${status} en ${urlStr.slice(0, 80)} — rotando UA.`);
        ...
        if (typeof XMLHttpRequest !== 'undefined') {
            ...
            XMLHttpRequest.prototype.send = function(...args) {
                // E2E-AUDIT FIX (2026-04-16): Skip Cortex monitoring for API management endpoints
                const _url = this._ctxUrl || '';
                if (!_cortexIgnorePatterns.some(p => _url.includes(p))) {
...
(function verifyBank() {
  ...
  exportedApp.generateChannelEntry = window.UI._internal_generateChannelEntry; })();
...
const UAPhantomEngine = (function () {
    ...
    function get(strategy = 'random') {
        ...
        if (range) {
            // OMEGA HARDEN — TIER PRESERVATION: leer del BANK ORIGINAL, no del
            // _epochPermutation. _epochPermutation es un shuffle GLOBAL del banco
            // que invalida los rangos del tierMap (los índices [111-120] del shuffle
            // ya no contienen los Android UAs originales). Para llamadas con strategy
            // específica, usar BANK garantiza que el UA devuelto pertenezca al tier solicitado.
            const [start, end] = range;
            return BANK[start + Math.floor(Math.random() * (end - start))];
        }
        return _epochPermutation[Math.floor(Math.random() * BANK_SIZE)];
    }
...
// ─────────────────────────────────────────────────────────────────────────────
    ...
    const PRE_ARMED_RESPONSE_BUILDER = {
        ...
        buildFallbackTags: function (channel, index) {
            ...
            // ── 🔴 FALLBACK 400: Bad Request Syntax Recovery ──
            // Causa más frecuente en IPTV: EXTHTTP con headers excesivos (>8KB) o
            // malformados que NGINX/WAF rechaza. Estrategia: strip progresivo.
            tags.push(`#EXT-X-APE-FALLBACK-ID:400_BAD_REQUEST_STRIP`);
            tags.push(`#EXT-X-APE-FALLBACK-400-STRATEGY:STRIP_HEADERS,UA_ROTATE,METHOD_SWITCH,CLEAN_RECONNECT`);
            tags.push(`#EXT-X-APE-FALLBACK-400-RETRY:3`);
            tags.push(`#EXT-X-APE-FALLBACK-400-STRIP-TO:User-Agent,Accept,Accept-Encoding,Connection`);
            ...
            // ── 🔴 FALLBACK 405: Method Not Allowed Recovery ──
            tags.push(`#EXT-X-APE-FALLBACK-ID:405_METHOD_NOT_ALLOWED`);
            tags.push(`#EXT-X-APE-FALLBACK-405-STRATEGY:METHOD_SWITCH,UA_ROTATE,XFF_ROTATE,CLEAN_RECONNECT`);
            tags.push(`#EXT-X-APE-FALLBACK-405-METHOD-CHAIN:GET,POST,HEAD,OPTIONS`);
            tags.push(`#EXT-X-APE-FALLBACK-405-RETRY:5`);
    ...
    function detectProfile(channel) {
        // AGENT-ROOT-CAUSE FIX 2026-04-12: Classification must be RESOLUTION-DRIVEN.
        // Previously only checked numeric fields (channel.height/bitrate/fps) that are
        // almost NEVER populated from IPTV providers → fell through to P5 for ALL channels.
        // Now we ALSO parse resolution keywords from tvg-name + group-title (where it lives).

        // 1) If explicit ape_profile already assigned by upstream classifier, trust it.
        if (channel && typeof channel.ape_profile === 'string' && /^P[0-5]$/.test(channel.ape_profile)) {
            return channel.ape_profile;
        }
        if (channel && typeof channel.profile === 'string' && /^P[0-5]$/.test(channel.profile)) {
            return channel.profile;
        }

        // 2) Numeric metadata (if populated — rare, but authoritative when present).
        const height = channel?.height
                     || parseInt((channel?.resolution || '').split('x')[1]) || 0;
        ...
        // 3) STRING-BASED keyword detection across name + group-title + url.
        //    This is the path that matters for 99% of IPTV providers.
        const haystack = [
            channel?.name,
            channel?.tvg_name, channel?.['tvg-name'],
            channel?.group, channel?.groupTitle, channel?.['group-title'], channel?.group_title,
            channel?.category, channel?.categoryName,
            channel?.url
        ].filter(Boolean).join(' ').toUpperCase();

        // P0 — 8K
        if (/\b(8K|4320P?|UHD2|8\s*K)\b/.test(haystack)) return 'P0';

        // P1 — 4K/UHD with premium signal (UHD*, UHD+, DOLBY VISION, DV, HDR10\+, 60FPS, ULTRA HD)
        if (/\b(4K\+|UHD\*|UHD\+|DOLBY\s*VISION|\bDV\b|HDR10\+|ULTRA\s*HD|60\s*FPS|120\s*FPS)\b/.test(haystack)) return 'P1';

        // P2 — 4K/UHD standard
        if (/\b(4K|UHD|2160P?|3840X2160)\b/.test(haystack)) return 'P2';

        // P3 — FHD 1080p
        if (/\b(FHD|FULL\s*HD|1080P?|1920X1080)\b/.test(haystack)) return 'P3';

        // P4 — HD 720p
        if (/\b(HD|720P?|1280X720)\b/.test(haystack) && !/\bSD\b/.test(haystack)) return 'P4';
    ...
    function generateGlobalHeader(channelsCount) {
        ...
        const _headerBase = `#EXTM3U x-tvg-url="" x-tvg-url-epg="" x-tvg-logo="" x-tvg-shift=0 catchup="flussonic" catchup-days="7" catchup-source="{MediaUrl}?utc={utc}&lutc={lutc}" url-tvg="" refresh="1800"
#EXT-X-VERSION:7
...
#EXT-X-START:TIME-OFFSET=-10.0,PRECISE=NO
...
#EXT-X-APE-SYNC-SLAVE:MASTER_ORIGIN_SYNC_V2`;
        ...
        // ⭐ OMEGA 5: Inyectar NIVEL_1 directivas del LAB
        const _nivel1 = (typeof window !== 'undefined' && window.APE_PROFILES_CONFIG?.nivel1Directives) || [];
        if (_nivel1.length > 0) {
            let n1block = '\n# ===== LAB NIVEL_1 CALIBRATED (' + _nivel1.length + ' directivas) =====';
            for (const d of _nivel1) {
                const tag = (d.tag || '').trim();
                const val = (d.value || '').trim();
                if (tag) n1block += '\n' + (tag.startsWith('#') ? '' : '#') + tag + (val ? ':' + val : '');
            }
            return _headerBase + n1block;
        }
        return _headerBase;
    }
    ...
    const EL_TARGETS = {
        ...
        P1: { res: '3840x2160', w: 3840, h: 2160, bitrate: ,  hdr: 'hdr10plus',     upscale: 'ai-4k-plus',  vmaf: 95 },
        ...
        P4: { res: '', w: 1920, h: 1080, bitrate: 25000000,  hdr: 'sdr',           upscale: 'lanczos',     vmaf: 93 },
    ...
    function build_proven_quality(cfg, profile) {
        ...
        return {
            ...
            'X-Video-Resolution-Native': cfg.resolution || '',
    ...
    function build_anti_noise_tags(cfg, profile) {
        ...
        const height = cfg.height || parseInt((cfg.resolution || '').split('x')[1]) || 1080;
    ...
    function build_exthttp(cfg, profile, index, sessionId, reqId) {
        ...
        const headers = {
            ...
            "X-Max-Resolution": cfg.resolution || '',
            ...
            "X-APE-RESOLUTION": cfg.resolution || '',
        ...
        // PhD-Master-Integrator FIX (2026-04-11): skip Cortex injection on
        // user-initiated connections. Prevents self-block during provider integration.
        if (typeof window !== 'undefined' &&
            !window.APE_USER_INITIATED &&
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // 🛡️ FRONTCDN REDIRECT SHIELD v2.0 — PRE-RESOLVE ENGINE
    // ═══════════════════════════════════════════════════════════════════════
    // PROBLEMA REAL (confirmado con curl):
    //   1. ky-tv.cc SIEMPRE devuelve 302 → beth/rob.frontcdn.me (tanto .ts como .m3u8)
    //   2. FrontCDN devuelve un M3U8 Media Playlist con segmentos RELATIVOS (/hls/...)
    //   3. ExoPlayer sigue el 302 pero resuelve /hls/... contra ky-tv.cc → 404 → pantalla negra
    //   4. Forzar .ts NO funciona: FrontCDN devuelve M3U8 (application/x-mpegurl) de todas formas
    //
    // SOLUCIÓN v2.0: Pre-resolver los 302 redirects ANTES de generar la lista.
    //   - Se ejecuta una sola vez antes de la generación
    //   - Captura la URL final de FrontCDN (http://beth.frontcdn.me/auth/TOKEN)
    //   - Pone esa URL directamente en la lista M3U8
    //   - ExoPlayer recibe la URL de FrontCDN directamente → sin redirects → reproduce ✅
    //   - Los tokens de FrontCDN duran hasta 8760h (1 año) = válido
    //
    // IDEMPOTENTE: Solo afecta servidores con FrontCDN detectado. TIVISION no se toca.
    // POLIMÓRFICO: Cada canal tiene su propio token y CDN host (beth, rob, etc.)
    // ═══════════════════════════════════════════════════════════════════════
    const FRONTCDN_FORCE_TS_HOSTS = [
        'ky-tv.cc',
        'kemotv.cc',
        // Agregar más servidores FrontCDN aquí a medida que se detecten
    ];

    function isFrontCDNHost(baseUrl, creds = null) {
        // Soporte universal para MILES de proveedores (Detección dinámica Frontend Probe)
        if (creds && creds._forceTS) return true;
        if (creds && creds._frontCDNHost) return true;
        
        if (!baseUrl || typeof baseUrl !== 'string') return false;
        const lower = baseUrl.toLowerCase();
        return FRONTCDN_FORCE_TS_HOSTS.some(h => lower.includes(h));
    }

    // ── Cache global de URLs pre-resueltas de FrontCDN ──
    // Estructura: { "http://ky-tv.cc:80/live/user/pass/175784": "http://beth.frontcdn.me:80/auth/TOKEN" }
    if (typeof window !== 'undefined' && !window.__APE_FRONTCDN_RESOLVE_CACHE__) {
        window.__APE_FRONTCDN_RESOLVE_CACHE__ = {};
    }

    /**
     * Pre-resuelve los redirects 302 de FrontCDN para TODOS los canales live
     * de servidores detectados. Se ejecuta UNA VEZ antes de la generación.
     * Anti-509: Serializado con delay de 50ms entre requests.
     * @param {Array} channels - Array de canales a procesar
     * @param {Object} credentialsMap - Mapa de credenciales por servidor
     * @param {Function} progressCb - Callback de progreso (current, total)
     * @returns {Promise<number>} - Número de URLs resueltas exitosamente
     */
    async function preResolveFrontCDNRedirects(channels, credentialsMap, progressCb) {
        // ═══════════════════════════════════════════════════════════════
        // v23.0 GUARDIA DE EJECUCIÓN ÚNICA — Anomalía A10 CORREGIDA
        // Evita doble ejecución v2.0+v3.0 que causaba 1992 fetch simultáneos,
        // rate limiting del servidor, y bucle infinito del Córtex JS.
        // ═══════════════════════════════════════════════════════════════
        const globalScope = typeof globalThis !== 'undefined' ? globalThis : (typeof window !== 'undefined' ? window : self);
        if (globalScope.__APE_FRONTCDN_RESOLVING__) {
            console.log('🛡️ [FrontCDN v3.1] Pre-resolución ya en curso — omitiendo ejecución duplicada');
            return globalScope.__APE_FRONTCDN_RESOLVE_PROMISE__ || 0;
        }
        globalScope.__APE_FRONTCDN_RESOLVING__ = true;
        globalScope.__APE_FRONTCDN_RESOLVE_PROMISE__ = (async () => {
        try {
        // ═══════════════════════════════════════════════════════════════

        if (!globalScope.__APE_FRONTCDN_RESOLVE_CACHE__) globalScope.__APE_FRONTCDN_RESOLVE_CACHE__ = {};
        const cache = globalScope.__APE_FRONTCDN_RESOLVE_CACHE__;
        let resolved = 0;
        let failed = 0;
        const frontCDNChannels = [];

        // Paso 1: Identificar canales que necesitan pre-resolución
        for (const channel of channels) {
            if (channel.type === 'movie' || channel.stream_type === 'movie' ||
                channel.type === 'series' || channel.stream_type === 'series') continue;

            const streamId = channel.stream_id || channel.id;
            if (!streamId) continue;

            // Buscar credenciales del servidor de este canal
            const sid = channel.serverId || channel._serverId || channel.__serverKey;
            let creds = null;
            if (sid && credentialsMap[sid]) creds = credentialsMap[sid];
            if (!creds) creds = credentialsMap['__current__'];
            if (!creds || !creds.baseUrl) continue;

            if (!isFrontCDNHost(creds.baseUrl, creds)) continue;

            const directUrl = `${creds.baseUrl}/live/${creds.username}/${creds.password}/${streamId}.ts`;
            if (cache[directUrl]) continue; // Ya resuelto

            frontCDNChannels.push({ channel, directUrl, streamId });
        }

        if (frontCDNChannels.length === 0) {
            console.log('🛡️ [FrontCDN v2.0] No hay canales FrontCDN para pre-resolver.');
            return 0;
        }

        console.log(`🛡️ [FrontCDN v4.0] ${frontCDNChannels.length} canales candidatos`);

        // ═══════════════════════════════════════════════════════════════
        // v4.0 PIPELINE: persistent cache (localStorage) + sample verify + parallel resolve
        // Objetivo: <1s si cache válido, ~17s si re-resolución completa (25 paralelos).
        // ═══════════════════════════════════════════════════════════════
        const PERSIST_KEY = 'APE_FRONTCDN_CACHE_v2';
        const CONCURRENCY = 25;
        const SAMPLE_SIZE = 20;
        const SAMPLE_INVALIDATE_RATIO = 0.20; // >=20% mismatches → invalidar cache

        // ── Helper: resolver 1 URL vía VPS (sin preflight) ──
        const _safeFetch = (typeof window !== 'undefined' && window._NATIVE_FETCH) ? window._NATIVE_FETCH : fetch;
        const vpsUrl = (typeof document !== 'undefined' && document.getElementById('vpsBaseUrl')?.value) || 'https://iptv-ape.duckdns.org';
        async function resolveOneFrontCDN(directUrl, timeoutMs = 5000) {
            const proxyUrl = `${vpsUrl}/resolve_redirect.php?url=${encodeURIComponent(directUrl)}`;
            const response = await _safeFetch(proxyUrl, {
                method: 'GET',
        ...
        // ── Paso 2a: Cargar cache persistente ──
        let persistCache = {};
        ...
        try {
            if (typeof localStorage !== 'undefined') {
                ...
                if (raw) {
                    ...
                    persistCache = parsed.resolutions || {};
                    persistMeta = { version: parsed.version, generatedAt: parsed.generatedAt, size: Object.keys(persistCache).length };
                }
            }
        } catch (e) {
            console.warn('🛡️ [FrontCDN v4.0] Cache persistente ilegible, continuo sin él:', e.message);
            persistCache = {};
        }
        if (persistMeta) {
            console.log(`🛡️ [FrontCDN v4.0] Cache persistente cargado: ${persistMeta.size} entradas (generado ${persistMeta.generatedAt})`);
        } else {
            console.log(`🛡️ [FrontCDN v4.0] Sin cache persistente previo (primera ejecución)`);
        }

        // ── Paso 2b: Sample verification (si hay >= SAMPLE_SIZE cacheados) ──
        const cachedHits = frontCDNChannels.filter(fc => persistCache[fc.directUrl]);
        let cacheInvalidated = false;
        if (cachedHits.length >= SAMPLE_SIZE) {
            if (progressCb) progressCb(0, frontCDNChannels.length, { phase: 'verify' });
            const sample = _shuffleSlice(cachedHits, SAMPLE_SIZE);
            console.log(`🛡️ [FrontCDN v4.0] Verificando cache con sample de ${SAMPLE_SIZE} canales…`);
            const verifyStart = Date.now();
            const results = await Promise.allSettled(sample.map(async ({ directUrl }) => {
                const fresh = await resolveOneFrontCDN(directUrl, 4000);
                return { directUrl, cached: persistCache[directUrl], fresh };
            }));
            let mismatches = 0, checks = 0;
            for (const r of results) {
                if (r.status === 'fulfilled' && r.value.fresh) {
                    checks++;
                    if (r.value.fresh !== r.value.cached) mismatches++;
                }
            }
            const mismatchRatio = checks > 0 ? (mismatches / checks) : 0;
            const dt = Date.now() - verifyStart;
            cacheInvalidated = mismatchRatio >= SAMPLE_INVALIDATE_RATIO;
            console.log(`🛡️ [FrontCDN v4.0] Verify: ${mismatches}/${checks} mismatches (${(mismatchRatio*100).toFixed(0)}%) en ${dt}ms → ${cacheInvalidated ? 'INVALIDAR' : 'VÁLIDO'}`);
            if (cacheInvalidated) {
                persistCache = {};
            }
        } else if (cachedHits.length > 0) {
            console.log(`🛡️ [FrontCDN v4.0] Cache parcial (${cachedHits.length}<${SAMPLE_SIZE}), se usa sin verificar`);
        }

        // ── Paso 2c: Split cached (hit) vs new (miss). Aplica también el bypass _frontCDNHost ──
        const toResolve = [];
        for (const fcdn of frontCDNChannels) {
            const { directUrl, channel } = fcdn;

            // Bypass 1: credencial pre-mapea al host final (ya existía en v3.1)
            const sid = channel.serverId || channel._serverId || channel.__serverKey;
            const creds = credentialsMap[sid] || credentialsMap['__current__'];
            if (creds && creds._frontCDNHost) {
                try {
                    const urlObj = new URL(directUrl);
                    urlObj.hostname = creds._frontCDNHost;
                    urlObj.protocol = 'http:';
                    const builtUrl = urlObj.toString();
                    cache[directUrl] = builtUrl;
                    persistCache[directUrl] = builtUrl;
                    resolved++;
                } catch (_) { /* noop */ }
                continue;
            }

            // Bypass 2: ya en cache persistente
            if (persistCache[directUrl]) {
                cache[directUrl] = persistCache[directUrl];
                resolved++;
                continue;
            }
        ...
        console.log(`🛡️ [FrontCDN v4.0] Split: ${resolved} cache-hit + ${toResolve.length} por resolver`);

        // ── Paso 2d: Resolución paralela de los nuevos (CONCURRENCY workers) ──
        if (toResolve.length > 0) {
            const resolveStart = Date.now();
            let completed = 0;
            let index = 0;
            let panic = false;
            const workerCount = Math.min(CONCURRENCY, toResolve.length);

            const workers = Array(workerCount).fill(null).map(async () => {
                while (!panic) {
                    const i = index++;
                    if (i >= toResolve.length) break;
                    const { directUrl, channel } = toResolve[i];
                    try {
                        const fresh = await resolveOneFrontCDN(directUrl);
                        if (fresh) {
                            cache[directUrl] = fresh;
                            persistCache[directUrl] = fresh;
                        } else {
                            cache[directUrl] = directUrl;
                        }
                        resolved++;
                    } catch (e) {
                        failed++;
                        cache[directUrl] = directUrl;
                        if (failed <= 3) {
                            console.warn(`🛡️ [FrontCDN v4.0] Error ${channel.name}: ${e.message}`);
                        }
                        // Panic budget más alto que serial (25 paralelos → umbral 25)
                        if (failed > 25) {
                            panic = true;
                            console.error('🛡️ [FrontCDN v4.0] Panic Stop: 25+ fallos, parando workers');
                        }
                    }
                    completed++;
                    if (progressCb && (completed % 50 === 0 || completed === toResolve.length)) {
                        progressCb(resolved, frontCDNChannels.length);
                    }
                }
            });
            await Promise.all(workers);
            const dt = Date.now() - resolveStart;
            console.log(`🛡️ [FrontCDN v4.0] Paralelo ${workerCount}w: ${completed}/${toResolve.length} en ${dt}ms`);
        }

        // ── Paso 2e: Persistir cache actualizado ──
        try {
            if (typeof localStorage !== 'undefined') {
                const payload = {
                    version: 2,
                    generatedAt: new Date().toISOString(),
                    resolutions: persistCache
                };
                localStorage.setItem(PERSIST_KEY, JSON.stringify(payload));
                console.log(`🛡️ [FrontCDN v4.0] Cache persistido: ${Object.keys(persistCache).length} entradas`);
            }
        } catch (e) {
            console.warn('🛡️ [FrontCDN v4.0] No se pudo persistir cache:', e.message);
        }

        console.log(`🛡️ [FrontCDN v4.0] ✅ Pre-resolución completa: ${resolved} OK, ${failed} fallidos de ${frontCDNChannels.length}`);
        return resolved;

        // ═══════════════════════════════════════════════════════════════
        // Cierre del semáforo de ejecución única (Paso 2 — A10)
        // ═══════════════════════════════════════════════════════════════
        } finally {
            globalScope.__APE_FRONTCDN_RESOLVING__ = false;
        }
        })(); // Cierre de la IIFE async del semáforo
        return globalScope.__APE_FRONTCDN_RESOLVE_PROMISE__;
    }

    /**
     * Busca la URL pre-resuelta de FrontCDN para un canal dado.
     * Si existe en cache, devuelve la URL de FrontCDN directamente.
     * Si no existe, devuelve null (se usará la URL original).
     */
    function getFrontCDNResolvedUrl(directUrl) {
        if (typeof window === 'undefined' || !window.__APE_FRONTCDN_RESOLVE_CACHE__) return null;
        return window.__APE_FRONTCDN_RESOLVE_CACHE__[directUrl] || null;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ANTES: Cambiaba https→http para ciertos puertos/hosts.
    // RESULTADO: HttpDataSource$HttpDataSourceException en ExoPlayer.
    // REGLA: El protocolo del servidor es SAGRADO. NUNCA se modifica.
    // Si el proveedor usa HTTP → HTTP. Si usa HTTPS → HTTPS.
    // ═══════════════════════════════════════════════════════════════════════
    ...
    function generateJWT68Fields(channel, profile, index) {
        // ═══════════════════════════════════════════════════════════════
        // v23.0 JWT REAL — 30+ claims compactos, alg:none (sin crypto)
        // Decodificable: JSON.parse(atob(token.split('.')[1]))
        // Viaja en: Authorization Bearer (CAPA C), Cookie (CAPA B)
        // ═══════════════════════════════════════════════════════════════
        const cfg = getProfileConfig(profile);
        const now = Math.floor(Date.now() / 1000);
        const jti = generateRandomString(16);
        const payload = {
            // Sección 1: Identificación (4 campos)
            iss: 'APE_v23.0', iat: now, exp: now + 86400, jti,
            // Sección 2: Canal (4 campos)
            chn: (channel.name || '').substring(0, 40),
            chn_id: String(channel.stream_id || channel.id || ''),
            chn_cat: String(channel.category_id || channel.category_name || '').substring(0, 20),
            chn_lang: channel.language || 'und',
            // Sección 3: Perfil compacto (4 campos — referencia P0-P5 = 500+ características implícitas)
            p: profile,
            res: cfg.resolution || '',
            fps: cfg.fps || 30,
            br: cfg.bitrate || 8000,
            // Sección 4: Calidad (6 campos)
            codec: cfg.codec_primary || 'HEVC',
            hdr: (cfg.hdr_support || []).join(','),
            cd: cfg.color_depth || 10,
            ach: cfg.audio_channels || 6,
            lcevc: cfg.lcevc ? 4 : 0,
            hevc_lvl: cfg.hevc_level || '5.1',
            // Sección 5: Buffer (4 campos)
            buf: cfg.buffer_ms || 60000,
            net: cfg.network_cache_ms || 8000,
            pf_seg: cfg.prefetch_segments || 3,
            pf_par: cfg.prefetch_parallel || 2,
            // Sección 6: Estrategia de ancho de banda (4 campos)
            t1: cfg.throughput_t1 || 35,
            t2: cfg.throughput_t2 || 75,
            bw_max: cfg.max_bandwidth || 150000000,
            vmaf: cfg.vmaf_target || 95,
            // Sección 7: Evasión (3 campos)
            ua_tier: 'phantom_v3',
            isp_ev: 'max',
            qos: 'EF',
            // Sección 8: Meta (3 campos)
            v: VERSION,
            arch: 'OMEGA_CRYSTAL_UNIVERSAL',
            idx: index,
        };
        // JWT unsigned: Header.Payload. (sin signature)
        ...
        return { token, payload, sessionId: jti, reqId: `${jti}-${index}` };
    }
    ...
    function build_stream_inf(cfg, channel) {
        ...
        const res = cfg.res || cfg.resolution || '';
...
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    // 🔴 FIX 2026-04-17: NUNCA usar channel.url directo como shortcut.
    // El API de Xtream devuelve URLs pre-tokenizadas (/live/play/TOKEN/id)
    // que NO funcionan para playback. SIEMPRE reconstruir desde credentialesMap
    // con username/password REALES. channel.url solo como ÚLTIMO RECURSO.
    function buildChannelUrl(channel, jwt, profile = null, index = 0, credentialsMap = {}) {
        // DEPRECATED: No usar channel.url como shortcut — ver fix 2026-04-17
        // Las URLs del API vienen tokenizadas y causan "clave incorrecta"
        ...
        if (existingUrl) existingUrl = getTierUrl(existingUrl);
        // 🔴 existingUrl se guarda como ÚLTIMO FALLBACK, NO se retorna aquí.
        // El flujo SIEMPRE intenta reconstruir desde credentialsMap primero.
        ...
        if (!creds || !creds.baseUrl || !creds.username || !creds.password) {
            // 🔴 FIX 2026-04-17: Si no hay credenciales en el map, usar existingUrl como último recurso
            if (existingUrl) {
                if (index < 5) console.warn(`⚠️ [buildChannelUrl] Sin creds para ${channel.name} (sid=${sid}) — usando channel.url como fallback`);
        ...
        // ═══════════════════════════════════════════════════════════════
        // v23.0 EXTENSIÓN DINÁMICA — Bug B6 CORREGIDO
        // Tabla de decisión por tipo de servidor + contenido
        // ═══════════════════════════════════════════════════════════════
        let typePath = 'live';
        let ext = 'ts'; // Default seguro
        if (channel.type === "movie" || channel.stream_type === "movie") {
            typePath = "movie";
            ...
            // Live: detectar extensión según tipo de servidor
            if (isFrontCDNHost(creds.baseUrl, creds)) {
                ext = 'ts'; // FrontCDN siempre .ts (evita CORS del redirect)
            } else if (creds._serverType === 'stalker') {
                ext = 'ts'; // Stalker/MAG solo sirve MPEG-TS
            } else if (typeof window !== 'undefined' && window.app?.state?.activeServers) {
                ...
                // PLAYBACK-COMPAT FIX 2026-04-12c: default .m3u8 not .ts (working list 0411 used .m3u8)
                else ext = 'm3u8';
            }
            // Override explícito del canal
            if (channel.customFormat) ext = channel.customFormat;
            if (channel.container_extension && channel.container_extension !== 'mp4') ext = channel.container_extension;
        }
        // ── FRONTCDN SHIELD v3.1: DIRECT GENERATION FALLBACK ──
        // Si el usuario generará la lista desde APE, usamos la URL pre-resuelta
        // con el token de auth (válido 1 año) en lugar de mandar todo al VPS.
        if (typePath === 'live' && isFrontCDNHost(creds.baseUrl, creds)) {
            const directUrl = `${creds.baseUrl}/live/${creds.username}/${creds.password}/${streamId}.ts`;
            const globalScope = typeof globalThis !== 'undefined' ? globalThis : (typeof window !== 'undefined' ? window : self);
            const cache = globalScope.__APE_FRONTCDN_RESOLVE_CACHE__;
            let resolvedUrl = cache && cache[directUrl];
            
            // QUICK FALLBACK: Si fetch opaqueredirect nos impidió leer Location, pero app.js conoce el host real
            if ((!resolvedUrl || resolvedUrl === directUrl) && creds._frontCDNHost) {
                try {
                    const urlObj = new URL(directUrl);
                    urlObj.hostname = creds._frontCDNHost;
                    urlObj.protocol = 'http:'; 
            ...
            if (resolvedUrl && resolvedUrl !== directUrl) {
                // Devolvemos la URL ya resuelta (Token puro o Host Proxy CDN)
                return resolvedUrl;
            }
            // Fallback total en caso que no haya resuelto
            ext = 'ts';
        }
        
        // ── USA v1.0: URL universal idempotente y polimórfica ──
...
// ═══════════════════════════════════════════════════════════════════════════
// v23.0 LIMPIEZA: __getOmegaGodTierDirectives() ELIMINADA (Deuda D5)
// Era código muerto — nunca llamada desde generateChannelEntry().
// Las directivas que generaba ya se emiten en L8 y L9.
// ═══════════════════════════════════════════════════════════════════════════
function __getOmegaGodTierDirectives_REMOVED(channel, cfg) {
    const extLines = [];
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // v23.0 CAPA B: Cookie Builder — Metadata comprimida como cookies
    // Servidor lee con $_COOKIE['ape_p'] o NGINX $cookie_ape_p
    // ═══════════════════════════════════════════════════════════════════════════
    function _buildApeCookie(cfg, profile, sid, classification) {
        const parts = [
            `ape_p=${profile}`,
            `ape_c=${(cfg.codec_primary || 'HEVC').substring(0,4)}`,
            `ape_r=${cfg.resolution || ''}`,
            `ape_f=${cfg.fps || 30}`,
            `ape_b=${cfg.bitrate || 8000}`,
            `ape_hdr=${(cfg.hdr_support || []).join('+')}`,
            `ape_lcevc=${cfg.lcevc ? '4' : '0'}`,
            `ape_sid=${sid}`,
            `ape_v=${VERSION}`,
            `ape_bw=${cfg.max_bandwidth || 0}`,
            `ape_buf=${cfg.buffer_ms || 60000}`,
            `ape_t1=${cfg.throughput_t1 || 0}`,
            `ape_t2=${cfg.throughput_t2 || 0}`,
            `ape_vmaf=${cfg.vmaf_target || 95}`,
            `ape_qos=EF`,
            `ape_isp=EXTREME`,
            `ape_cd=${cfg.color_depth || 10}`,
            `ape_ach=${cfg.audio_channels || 6}`,
            `ape_hevc_lvl=${cfg.hevc_level || '5.1'}`,
            `ape_pf_seg=${cfg.prefetch_segments || 3}`,
        ];
        if (classification) {
            parts.push(`ape_reg=${classification.region?.code || 'XX'}`);
            parts.push(`ape_cat=${(classification.category?.group || 'GEN').substring(0,6)}`);
            parts.push(`ape_lang=${classification.language?.code || 'und'}`);
            if (classification.sportSubcategory) parts.push(`ape_sport=${classification.sportSubcategory}`);
        }
        return parts.join('; ');
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // v23.0 CAPA E: Overflow Headers Builder — Agrega ~460 headers en base64
    // Runtime Evasion Engine (ape-runtime-evasion-engine.js) YA decodifica este
    // blob y los inyecta en cada HTTP request en runtime.
    // ═══════════════════════════════════════════════════════════════════════════
    function _buildOverflowHeaders(cfg, profile, index) {
        const overflow = {};

        // Enhancement Layer (75 keys: X-EL-*)
        try { Object.assign(overflow, build_enhancement_layer(cfg, profile)); } catch(e) {}

        // Proven Quality (140 keys: X-Video-*, X-HEVC-*, X-x265-*, X-SVT-AV1-*, X-HDR10-*, X-Dolby-*, X-Samsung-*, X-HLG-*, X-Tone-*, X-Noise-*, X-Film-*, X-VMAF-*)
        try { Object.assign(overflow, build_proven_quality(cfg, profile)); } catch(e) {}

        // AntiFreeze Nuclear (83 keys: X-AF-*)
        try { Object.assign(overflow, build_antifreeze_nuclear_headers(cfg, profile)); } catch(e) {}

        // HTTP Anabolic Demo (~100 keys)
        try { Object.assign(overflow, build_http_anabolic_demo_headers(cfg, cfg._channel || {})); } catch(e) {}

        // Anabolized Headers (~150 keys)
        try { Object.assign(overflow, build_anabolized_headers(cfg, cfg._channel || {})); } catch(e) {}

        // ISP Throttle headers
        if (ACTIVE_ISP_LEVEL) {
            overflow['X-ISP-Throttle-Level'] = `1-${ACTIVE_ISP_LEVEL.name}`;
            overflow['X-ISP-BW-Demand'] = ACTIVE_ISP_LEVEL.strategy || 'MAX_CONTRACT';
            overflow['X-ISP-BW-Ceiling'] = 'NONE';
            overflow['X-ISP-BW-Floor'] = 'FULL_CONTRACT';
            overflow['X-ISP-Burst-Mode'] = 'ENABLED';
            overflow['X-ISP-Burst-Factor'] = `${ACTIVE_ISP_LEVEL.burst_factor || 1.5}x`;
            overflow['X-ISP-Parallel-Streams'] = String(ACTIVE_ISP_LEVEL.parallel_streams || 4);
            overflow['X-ISP-TCP-Window'] = String((ACTIVE_ISP_LEVEL.tcp_window_mb || 4) * 1024 * 1024);
            overflow['X-ISP-TCP-Nodelay'] = '1';
            overflow['X-ISP-TCP-Quickack'] = '1';
            overflow['X-ISP-QUIC-Enabled'] = String(ACTIVE_ISP_LEVEL.quic !== false);
            overflow['X-ISP-HTTP3-Enabled'] = String(ACTIVE_ISP_LEVEL.http3 !== false);
        }

        // Telchemy QoS diagnostics
        overflow['X-Telchemy-VSTQ-Target'] = '50';
        overflow['X-Telchemy-VSMQ-Target'] = '50';
        overflow['X-Telchemy-EPSNR-Min'] = '45';
        overflow['X-Telchemy-MAPDV-Max'] = '10';
        overflow['X-Telchemy-PPDV-Max'] = '5';
        overflow['X-Telchemy-TR101290-Sync-Loss'] = '0';
        overflow['X-Telchemy-TR101290-CC-Error'] = '0';
        overflow['X-Telchemy-Blockiness-Guard'] = 'ACTIVE';
        overflow['X-Telchemy-Jerkiness-Guard'] = 'ACTIVE';
        overflow['X-Telchemy-Buffer-Underrun-Action'] = 'EXPAND_NETWORK_CACHING';

        // ITM SDR→HDR Engine
        overflow['X-ITM-Mode'] = 'ADAPTIVE_FRAME_BY_FRAME';
        overflow['X-ITM-Target-Nits'] = '1000';
        overflow['X-ITM-Target-Gamut'] = 'BT.2020';
        overflow['X-ITM-Analysis'] = 'LUMINANCE_PEAK+APL+HISTOGRAM';
        overflow['X-ITM-Metadata-Injection'] = 'MaxFALL+MaxCLL+DYNAMIC';
        overflow['X-Cortex-ITM-Engine'] = 'ENABLED_SDR_ONLY';
        overflow['X-Cortex-ITM-Analysis-Depth'] = 'FULL_HISTOGRAM';
        overflow['X-Cortex-ITM-Expansion-Curve'] = 'ADAPTIVE_PER_FRAME';
        overflow['X-Cortex-ITM-Color-Space-Out'] = 'BT.2020_NCL';
        overflow['X-Cortex-ITM-Bit-Depth-Out'] = '10bit';

        // PEVCE Player Enforcement
        overflow['X-PEVCE-Network-Caching'] = '3000';
        overflow['X-PEVCE-Live-Caching'] = '3000';
        overflow['X-PEVCE-TCP-Caching'] = '3000';
        overflow['X-PEVCE-HTTP-Reconnect'] = 'true';
        overflow['X-PEVCE-HTTP-Continuous'] = 'true';
        overflow['X-PEVCE-HW-Decode'] = 'any';
        overflow['X-PEVCE-HW-Threads'] = '0';
        overflow['X-PEVCE-Force-HDR'] = 'true';

        // Classification metadata
        if (cfg._classification) {
            const cl = cfg._classification;
            overflow['X-APE-CL-Region'] = cl.region?.code || 'XX';
            overflow['X-APE-CL-Country'] = cl.region?.country || 'Desconocido';
            overflow['X-APE-CL-Content-Type'] = cl.contentType?.type || 'GENERALISTA';
            overflow['X-APE-CL-Language'] = cl.language?.code || 'unknown';
            overflow['X-APE-CL-Confidence'] = String(Math.round((cl.confidence || 0) * 100));
        }

        // Channel identification
        if (cfg._channelName) overflow['X-APE-Channel-Name'] = cfg._channelName;
        if (cfg._channelId) overflow['X-APE-Channel-ID'] = cfg._channelId;
    ...
    // L0-L10 | 6 Capas de Transporte | 0 headers perdidos
    // Pipeline EXTHTTP unificado + Cookie + JWT + OVERFLOW base64
    // ═══════════════════════════════════════════════════════════════════════════
    function generateChannelEntry(channel, profile = 'P3', index = 0, credentialsMap = {}) {
        ...
        // LIST_HASH: hash global de la lista que el caller (window.LIST_HASH) puede inyectar
        // antes de la generación para que TODOS los canales compartan el mismo identificador.
        // Si no se define, cae al fallback per-channel _nonce796.slice(0,8).
        // El fallback garantiza que el header X-APE-List-Hash siempre tenga valor (no 'undefined').
        ...
        // OMEGA HARDEN: tierMap del UAPhantomEngine usa keys camelCase ('Android', 'iOS',
        // 'Safari', 'Kodi', 'TiviMate'). La API expuesta NO incluye getUA(); solo get().
        // Llamadas previas con MAYÚSCULAS o getUA() caían silenciosamente al fallback random,
        // produciendo X-User-Agent-* labels desincronizados de sus User Agents reales.
        // Decisión sobre 'CHROME'/'EXOPLAYER': mapear al rango más cercano del tierMap
        // sin refactorizar el engine ('Windows' = Chrome desktop, 'Android' = ExoPlayer en Android).
        const _ua796        = UAPhantomEngine.get('IPTV');
        const _uaAndroid    = UAPhantomEngine.get('Android');
        const _uaIOS        = UAPhantomEngine.get('iOS');
        const _uaChrome     = UAPhantomEngine.get('Windows');
        const _uaTiviMate   = UAPhantomEngine.get('TiviMate');
        const _uaOTT        = UAPhantomEngine.get('OTT');
        const _uaKodi       = UAPhantomEngine.get('Kodi');
        const _uaExo        = UAPhantomEngine.get('Android');
        ...
        const jwt           = (typeof generateJWT68Fields === 'function')
                              ...
                              : null;
        // OMEGA HARDEN: jwt puede ser null O un string stub ("JWT_STUB").
        // Solo usamos sus campos si es un objeto real con las propiedades esperadas.
        const sessionId     = (jwt && typeof jwt === 'object' && jwt.sessionId) ? jwt.sessionId : (_sid796 + '_' + _nonce796);
        const reqId         = (jwt && typeof jwt === 'object' && jwt.reqId)     ? jwt.reqId     : (_nonce796 + index.toString(16));

        // ── v23.0: build_exthttp() ELIMINADO — pipeline unificado en L2 ──
        // Los headers de build_exthttp() ahora se generan directamente:
        //   - Funcionales (~30) → _httpFunctional en L2
        //   - Overflow (~460) → _buildOverflowHeaders() → base64 en L2
        ...
        // ── USA v1.0: fingerprint + overrides para este canal ──
        ...
        // ═══════════════════════════════════════════════════════════════
        // v23.0 URL BUILDER — Deuda D4 CORREGIDA
        // Prioridad 1: buildUniversalUrl() (USA Module — detección de servidor)
        // Prioridad 2: buildChannelUrl() (legacy fallback)
        // ═══════════════════════════════════════════════════════════════
        let primaryUrl = '';
        try {
            primaryUrl = buildUniversalUrl(channel, _usaCredsForChannel, _usaFP, profile);
        } catch (eUsa) {}
        if (!primaryUrl) {
        ...
        // URL-AUDIT FIX (2026-04-16): If BOTH URL builders failed, skip this channel
        // entirely rather than emitting an EXT-X-STREAM-INF with an empty URL line.
        // Empty URLs cause player parse errors and waste bytes in the list.
        if (!primaryUrl) {
            if (index < 10) console.warn(`⚠️ [generateChannelEntry] Canal "${channel.name || index}" sin URL — omitido (sid=${channel.serverId || 'none'}, stream_id=${channel.stream_id || 'none'})`);
            return '';
        }

        // ═══════════════════════════════════════════════════════════════
        // v23.0 URL LIMPIA — Bug B5 + A7 CORREGIDO
        // Los parámetros APE (ape_sid, ape_nonce, profile) ya NO viajan en la URL.
        // Viajan en: CAPA A (EXTHTTP X-APE-SID), CAPA B (Cookie ape_sid=),
        //           CAPA C (JWT jti), CAPA D (KODIPROP network_session_id),
        //           CAPA F (EXT-X-APE-* tags L7/L8)
        // Motivo: El 95% de servidores Xtream Codes rechazan parámetros
        //         desconocidos con HTTP 400/403 (Bug B5 confirmado).
        // ═══════════════════════════════════════════════════════════════

        // Helper: limpia la URL de cualquier parámetro APE residual
        ...
        // Limpiar URL primaria de parámetros APE
        if (primaryUrl) {
        ...
        // ═══════════════════════════════════════════════════════════════
        // 🎯 Plan Supremo Moderno — Bucket A/B/C + fail-closed por modo
        // ═══════════════════════════════════════════════════════════════
        const _pubMode = String((typeof window !== 'undefined' && window.APE_PUBLICATION_MODE) || 'balanced_95').toLowerCase();
        const _pubTier = (_admission.entry && _admission.entry.publicationTier) || (_admission.entry ? 'A' : 'B');
        const _expectedStatus = (_pubTier === 'A') ? 'terminal_200' : 'redirect_transition';
        const _admissionSource = _admission.entry ? 'health_checker' : 'fallback_panel';
        const _validatedAt = (_admission.entry && _admission.entry.validatedAt) || '';
        const _observedRole = (_admission.entry && _admission.entry.observedRole) || 'playlist_hls';
        const _stabilityScore = (_admission.entry && _admission.entry.stabilityScore) || 0;

        // Strict Terminal: solo emitir Bucket A.
        if (_pubMode === 'strict_terminal' && _pubTier !== 'A') {
            if (index < 25) console.warn(`🔒 [STRICT] Canal omitido (tier=${_pubTier}): ${channel.name || channel.stream_id || index}`);
            return '';
        }
        ...
        // ════════════════════════════════════════════════════════════════════════
        // L0 — IDENTIDAD HLS (1 línea: solo EXTINF)
        // ════════════════════════════════════════════════════════════════════════
        // BUG FIX 2026-04-10 (3 bugs fatales RFC 8216):
        //   1. #EXT-X-VERSION:7 — ya está en el header global (generateGlobalHeader).
        //      Repetirlo por canal causa que parsers de estado finito reinicien su máquina.
        //   2. #EXT-X-TARGETDURATION, #EXT-X-MEDIA-SEQUENCE, #EXT-X-PLAYLIST-TYPE —
        //      son exclusivas de Media Playlists (sub-manifiestos .m3u8 del proveedor).
        //      Inyectarlas en un Master Playlist viola RFC 8216 §4.3.3 y causa que
        //      ExoPlayer, hls.js, y Safari rechacen el archivo COMPLETO.
        //   3. #EXT-X-STREAM-INF sin URL en la línea siguiente — ExoPlayer lee el
        //      STREAM-INF de L0 (línea 77) y busca URL en L1 (línea 78), pero encuentra
        //      #EXTVLCOPT:network-caching=60000 → descarta el canal. Los STREAM-INF
        //      reales están en L10, correctamente pareados con sus URLs.
        lines.push(_extinf);

        // ── Plan Supremo Moderno — Marca de publicación por canal ────────────
        // Permite al auditor separar Bucket A/B/C y medir tier ratios.
        // No interfiere con parseo HLS: los reproductores ignoran #EXT-X-APE:* desconocido.
        {
            const _tierParts = [
                `PUBLICATION-TIER=${_pubTier}`,
                `EXPECTED-STATUS=${_expectedStatus}`,
                `ADMISSION-SOURCE=${_admissionSource}`,
                `OBSERVED-ROLE=${_observedRole}`,
                `MODE=${_pubMode}`
            ];
            if (_validatedAt) _tierParts.push(`VALIDATED-AT=${_validatedAt}`);
            if (_stabilityScore) _tierParts.push(`STABILITY=${_stabilityScore}`);
            lines.push(`#EXT-X-APE:${_tierParts.join(';')}`);
        }
        ...
        // ════════════════════════════════════════════════════════════════════════
        // ── USA v1.0: EXTVLCOPT polimórficos ──
        ...
        // SUPERAGENT PATCH 2 (2026-04-11): unconditional playback recovery directives
        // for VLC/TiviMate/OTTNav — http-max-retries must be EXTVLCOPT (not EXTHTTP)
        lines.push(`#EXTVLCOPT:http-max-retries=99`);
        ...
        // ── Params HDR/Color extendidos (desde APE_PROFILES_CONFIG) ──
        const _pmCfgHdr = (typeof window !== 'undefined' && window.APE_PROFILES_CONFIG) ? window.APE_PROFILES_CONFIG.getProfile(profile) : null;
        const _pmSettings = _pmCfgHdr?.settings || {};
        const _pmVlcopt = _pmCfgHdr?.vlcopt || {};
        const _colorSpace = _pmSettings.color_space || cfg.color_space || 'BT.2020';
        const _colorTransfer = _pmSettings.color_transfer || cfg.transfer_function || 'SMPTE ST 2084 (PQ)';
        const _colorPrimaries = _pmSettings.color_primaries || cfg.color_primaries || 'BT.2020';
        const _bitDepth = _pmSettings.bit_depth || cfg.color_depth || 10;
        const _chroma = _pmSettings.chroma_subsampling || cfg.chroma_subsampling || '4:2:0';
        const _codecFull = _pmSettings.codec_full || cfg.codec || _codec796;
        const _codecPriority = _pmSettings.codec_priority || cfg.codec_priority || 'hvc1,hev1,avc1';
        const _audioCodec = _pmSettings.audio_codec || cfg.audio_codec || 'mp4a.40.2';
        const _segDuration = _pmSettings.segment_duration_s || cfg.segment_duration || 6;
        lines.push(`#EXTVLCOPT:video-color-space=${_colorSpace}`);
        lines.push(`#EXTVLCOPT:video-color-transfer=${_colorTransfer}`);
        lines.push(`#EXTVLCOPT:video-color-primaries=${_colorPrimaries}`);
        lines.push(`#EXTVLCOPT:video-bit-depth=${_bitDepth}`);
        lines.push(`#EXTVLCOPT:video-chroma-subsampling=${_chroma}`);
        lines.push(`#EXTVLCOPT:codec-priority=${_codecPriority}`);
        lines.push(`#EXTVLCOPT:audio-codec-preferred=${_audioCodec}`);
        ...
        // AGENT-3 FIX 2026-04-11: emit file-caching + disc/tcp/sout mirrors (missing in prev gen)
        lines.push(`#EXTVLCOPT:file-caching=${_buf796}`);
        lines.push(`#EXTVLCOPT:disc-caching=${_buf796}`);
        lines.push(`#EXTVLCOPT:tcp-caching=${_buf796}`);
        lines.push(`#EXTVLCOPT:sout-mux-caching=${_buf796}`);
        ...
        if (window.ApeRealtimeEngine) {
          window.ApeRealtimeEngine.injectVlcopt(lines, profile);
        }
        // ─────────────────────────────────────────────────────────────────────────

        // ════════════════════════════════════════════════════════════════════════
        // L2 — EXTHTTP UNIFICADO v23.0 — 3 Sub-capas, 0 headers perdidos
        // ════════════════════════════════════════════════════════════════════════
        // SUB-CAPA A: ~30 headers HTTP funcionales (<3KB) → #EXTHTTP JSON
        // SUB-CAPA B: ~50 campos metadata comprimida → Cookie header (dentro de A)
        // SUB-CAPA C: ~30 claims JWT → Authorization header (dentro de A)
        // SUB-CAPA E: ~460 headers restantes → #EXT-X-APE-OVERFLOW-HEADERS base64
        //             Runtime Evasion Engine los decodifica e inyecta en runtime
        // ════════════════════════════════════════════════════════════════════════

        // ── Derivar Origin/Referer del servidor REAL del canal ──
        const _channelOrigin = (() => { try { return new URL(primaryUrl).origin; } catch(e) { return 'https://www.google.com'; } })();
        const _channelReferer = _channelOrigin + '/';

        // ── JWT Token (CAPA C) ──
        const _jwtToken = (jwt && typeof jwt === 'object' && jwt.token) ? jwt.token : '';

        // ── SUB-CAPA A: Headers HTTP funcionales que el servidor REALMENTE procesa ──
        const _httpFunctional = {
            // Identidad (el servidor/WAF procesa estos)
            'User-Agent':              _ua796,
            ...
            // Referencia (bypass hotlink protection)
            'Referer':                 _channelReferer,
            'Origin':                  _channelOrigin,
            // Browser fingerprint emulation (evasión WAF)
            ...
            // Identidad polimórfica (sesión)
            'X-Playback-Session-Id':   sessionId,
            'X-Request-Id':            reqId,
            'X-APE-SID':               _sid796,
            'X-APE-Nonce':             _nonce796,
            'X-APE-Profile':           profile,
            // UA rotation para Phantom Hydra (el player rota entre estos)
            ...
            // CAPA C: JWT real en Authorization
            ...(_jwtToken ? { 'Authorization': `Bearer ${_jwtToken}` } : {}),
            // CAPA B: Cookie con metadata comprimida (~50 campos)
            'Cookie':                  _buildApeCookie(cfg, profile, _sid796, channel._classification || null),
        };

        // ── Merge headerOverrides del perfil (X-Stream-*, X-HDR-*, X-Color-*, etc) ──
        // Los headers definidos en el profile JSON se inyectan en EXTHTTP antes del Engine.
        try {
            const _pmProf = (typeof window !== 'undefined' && window.APE_PROFILES_CONFIG)
                ? window.APE_PROFILES_CONFIG.getProfile(profile) : null;
            const _pmHeaderOverrides = _pmProf?.headerOverrides || {};
            if (_pmHeaderOverrides && typeof _pmHeaderOverrides === 'object') {
                Object.assign(_httpFunctional, _pmHeaderOverrides);
            }
            // ⭐ OMEGA 5: Merge NIVEL_3 HTT del LAB (headers HTTP extra calibrados)
            const _labHTT = (window.APE_PROFILES_CONFIG?.nivel3PerLayer?.HTT) || [];
            for (const item of _labHTT) {
                const k = (item.key || '').trim(), v = (item.value || '').trim();
                if (k && v) _httpFunctional[k] = v;
            }
        } catch (eHO) { /* silencioso */ }

        // ── AGENT-9/10/12/13 FIX 2026-04-12: Guarantee PhD headers embed (never undef) ──
        // Estos headers DEBEN existir en cada canal para cumplir las garantías 8-13.
        // Si el JSON del perfil no los trae, los inyectamos aquí con valores por default.
        try {
            const _pRes = (typeof APE_PROFILE_MATRIX !== 'undefined' && APE_PROFILE_MATRIX[profile])
                ? APE_PROFILE_MATRIX[profile]._meta : null;
            const _resStr = _pRes?.resolution || 'unknown';
            const _hdrStr = _pRes?.codec?.includes('dvh1') ? 'DOLBY_VISION_P8'
                          : _pRes?.codec?.includes('hvc1.2.4.L186') ? 'DOLBY_VISION_P8'
                          : _pRes?.codec?.includes('hvc1.2.4.L153') ? 'HDR10'
                          : _pRes?.codec?.includes('hvc1') ? 'HLG' : 'SDR';
            if (!_httpFunctional['X-Error-407-Taxonomy']) {
                _httpFunctional['X-Error-407-Taxonomy'] = 'V1:Basic,V2:Digest,V3:NTLM,V4:Negotiate,V5:OAuth,V6:Token,V7:mTLS,V8:Captive';
            }
            if (!_httpFunctional['X-Auth-Stub-Basic']) {
                _httpFunctional['X-Auth-Stub-Basic'] = 'Basic YW5vbjphbm9u';
            }
            if (!_httpFunctional['X-MultiProvider-Mode']) {
                _httpFunctional['X-MultiProvider-Mode'] = 'xtream,stalker,mag,m3u,enigma2,tvheadend';
            }
            if (!_httpFunctional['X-Provider-Detection']) {
                _httpFunctional['X-Provider-Detection'] = 'auto,url-inspection,api-probe,header-probe';
            }
            if (!_httpFunctional['X-Fatal-Error-Trap']) {
                _httpFunctional['X-Fatal-Error-Trap'] = 'enabled,catch-all,log-and-recover';
            }
            if (!_httpFunctional['X-Worker-Crash-Recovery']) {
                _httpFunctional['X-Worker-Crash-Recovery'] = 'respawn,state-preserve,max:5';
            }
            if (!_httpFunctional['X-Master-Audit-PhD']) {
                _httpFunctional['X-Master-Audit-PhD'] = `calibrated=true,profile=${profile},res=${_resStr},hdr=${_hdrStr},guarantees=10/10`;
            }
        } catch (ePhD) { /* silencioso */ }

        // ── APE REALTIME ENGINE v10.0 — Enriquecimiento de #EXTHTTP ──────────────
        // Inyecta CMCD (CTA-5004), QoE (Mux Data), ABR multi-motor, player configs.
        // Cap de seguridad: si el JSON resultante excede 8KB, los headers del Engine
        // se mueven al overflow (CAPA E) para que el Runtime Evasion Engine los inyecte.
        // MANDATORY_KEEP at function scope so both cap blocks can reference it.
        const MANDATORY_KEEP = new Set([
          ...
          'X-ABR-Stability', 'X-Stream-Bitrate', 'X-Stream-Codecs',
          'X-HDR-Mode', 'X-Color-Space',
          // AGENT-12/13 FIX 2026-04-12: PhD taxonomy + multi-provider + traps + master seal
          'X-Error-407-Taxonomy', 'X-Auth-Stub-Basic', 'Proxy-Authorization',
          'X-MultiProvider-Mode', 'X-Provider-Detection',
          'X-Fatal-Error-Trap', 'X-Worker-Crash-Recovery',
          'X-Master-Audit-PhD',
        ]);
        let _realtimeOverflow = null;
        if (window.ApeRealtimeEngine) {
          const _preInjectKeys = new Set(Object.keys(_httpFunctional));
          window.ApeRealtimeEngine.injectHeaders(_httpFunctional, profile, channel.stream_id || channel.id || '');
          const _extHttpTest = JSON.stringify(_httpFunctional);
          if (_extHttpTest.length > 10240) {
            _realtimeOverflow = {};
            for (const key of Object.keys(_httpFunctional)) {
              if (!_preInjectKeys.has(key) && !MANDATORY_KEEP.has(key)) {
                _realtimeOverflow[key] = _httpFunctional[key];
                delete _httpFunctional[key];
              }
            }
          }
        }
        ...
        // AGENT-11 UNIVERSAL CAP 2026-04-12: Final size check post all injections.
        // If EXTHTTP still >10KB, move ALL non-critical headers to overflow based
        // on a broad prefix denylist (X-Master/X-Error/X-Error-407/X-SelfBlock/
        // X-Playback/X-AntiFreeze/X-QoS/X-LiveSync/X-Buffer/X-CircuitBreaker/etc).
        // These are informational metadata already duplicated as #EXT-X-APE-* tags.
        if (JSON.stringify(_httpFunctional).length > 10240) {
          if (!_realtimeOverflow) _realtimeOverflow = {};
          const _evictPrefixes = [
            'X-Master-', 'X-Error-', 'X-SelfBlock-', 'X-Playback-',
            'X-AntiFreeze-', 'X-QoS-', 'X-LiveSync-', 'X-Buffer-',
            'X-CircuitBreaker-', 'X-Whitelist-',
            'X-Protocol-Support', 'X-Credentials-Isolation', 'X-Parallel-',
            'X-Cert-Validation', 'X-User-Initiated-',
            'X-SlotReaper-', 'X-Stalker-', 'X-MAG-', 'X-DASH-', 'X-Smooth-',
            'X-CDN-', 'X-Nudge-', 'X-Manual-', 'X-Profile-Tier',
            'X-Profile-Internal-', 'X-Profile-Boundary-', 'X-ABR-Scope',
            'X-Conflict-',
            'X-Manifesto-', 'X-Regression-', 'X-Agents-Consolidated',
            'X-Zero-Black-Screen', 'X-Bitrate-Ladder-Spacing', 'X-Bitrate-Ladder-Tiers',
            'X-HLSjs-Events-', 'X-HLSjs-ErrorDetails-', 'X-HLSjs-Telemetry',
            'X-HLSjs-Recovery', 'X-HLSjs-State', 'X-HLSjs-Error-Penalty',
            'X-CMCD-Mode', 'X-CMSD-Support', 'X-CMCD-Spec',
            'X-Dolby-Vision-', 'X-Dolby-Atmos', 'X-Max-Framerate', 'X-Min-Framerate',
            'X-Frame-Rate-Adaptive', 'X-Pixel-Aspect-Ratio', 'X-Display-Aspect-Ratio',
            'X-Video-Range', 'X-Video-Full-Range', 'X-Chroma-Sample-Position',
            'X-Transfer-Characteristics', 'X-Matrix-Coefficients', 'X-Color-Primaries-Code',
            'X-HDR-Metadata-Type', 'X-Max-Luminance', 'X-Min-Luminance',
            'X-HDR-Mode-Primary', 'X-HDR-Mode-Fallback', 'X-Segment-Duration-S',
          ];
          for (const key of Object.keys(_httpFunctional)) {
            if (MANDATORY_KEEP && MANDATORY_KEEP.has(key)) continue;
            if (_evictPrefixes.some(p => key.startsWith(p) || key === p)) {
              _realtimeOverflow[key] = _httpFunctional[key];
              delete _httpFunctional[key];
              if (JSON.stringify(_httpFunctional).length <= 10240) break;
            }
          }

          // AGENT-AUDIT FIX 2026-04-12b: LAST-RESORT cap enforcement.
          // If AFTER prefix-based eviction we're STILL >8KB, evict ANY X-* custom
          // header not in MANDATORY_KEEP until size drops under 8KB.
          // OkHttp/TiviMate starts truncating/rejecting headers beyond ~8KB total.
          const HARD_CAP_BYTES = 8192;
          if (JSON.stringify(_httpFunctional).length > HARD_CAP_BYTES) {
            if (!_realtimeOverflow) _realtimeOverflow = {};
            const keysCopy = Object.keys(_httpFunctional);
            // Evict X-* customs first (keep standard HTTP headers)
            for (const key of keysCopy) {
              if (MANDATORY_KEEP && MANDATORY_KEEP.has(key)) continue;
              if (!key.startsWith('X-') && !key.startsWith('CMCD-')) continue;
              _realtimeOverflow[key] = _httpFunctional[key];
              delete _httpFunctional[key];
              if (JSON.stringify(_httpFunctional).length <= HARD_CAP_BYTES) break;
            }
          }
        }

        // ── OKHTTP/TiviMate FIX 2026-04-12 ────────────────────────────────────
        // "Beautiful Madness 4-layer" pattern (comma-joined values) BREAKS strict
        // HTTP parsers like OkHttp/TiviMate/Kodi/ExoPlayer → "unexpected end of
        // stream on com.android.okhttp.Address@..." mid-playback.
        // For RFC 7230 protocol-level headers, keep ONLY the first token.
        // (User-Agent, Cookie, Referer, Accept-* tolerate multi-value and stay 4-layer.)
        const _SINGLE_VALUE_HEADERS = new Set([
            ...
            // AGENT-AUDIT FIX 2026-04-12b: Referer/Origin 4-layer values invalidate
            // HTTP requests in OkHttp/TiviMate/OTT Navigator. Force single-value.
            'Referer', 'Origin', 'Authorization', 'Range',
            'Accept-Encoding', 'Sec-CH-UA-Mobile', 'Sec-CH-UA-Platform',
        ]);
        for (const _h of Object.keys(_httpFunctional)) {
            if (_SINGLE_VALUE_HEADERS.has(_h) && typeof _httpFunctional[_h] === 'string'
                && _httpFunctional[_h].indexOf(',') !== -1) {
                // Referer/Origin: pick the FIRST full URL before the first comma
                _httpFunctional[_h] = _httpFunctional[_h].split(',')[0].trim();
            }
        }

        // AGENT-AUDIT FIX 2026-04-12b: Remove spoofed IP headers that backfire
        // for native players (TiviMate/OTT/VLC send from Android, not browser).
        // These were for browser-side CORS evasion; on native HTTP they trigger
        // WAF suspicion without any benefit.
        delete _httpFunctional['X-Forwarded-For'];
        delete _httpFunctional['X-Real-IP'];
        delete _httpFunctional['X-Client-IP'];
        delete _httpFunctional['CF-Connecting-IP'];
        delete _httpFunctional['True-Client-IP'];

        // ── PLAYBACK-COMPAT FIX 2026-04-12c (from working list 0411) ──────────
        // The 0411 list reproduced perfectly with these values. The profile JSON
        // overrides them with values that look "premium" but break OkHttp/CDN.
        // Force back the proven-working values just before emission.
        // 1) UA: real player UA, never the fake "APE-NAVIGATOR ULTRA-8K-HEVC-MASTER"
        if (typeof _httpFunctional['User-Agent'] === 'string'
            && /APE-NAVIGATOR|ULTRA-8K-HEVC-MASTER/i.test(_httpFunctional['User-Agent'])) {
            _httpFunctional['User-Agent'] =
                'Mozilla/5.0 (Web0S; Linux/SmartTV) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.34 Safari/537.36 WebAppManager';
        }
        // 2) Referer/Origin: never our own iptv-ape.duckdns.org (no streaming CDN
        //    expects that). Use neutral, well-known referer that worked before.
        if (typeof _httpFunctional['Referer'] === 'string'
            && _httpFunctional['Referer'].indexOf('iptv-ape.duckdns.org') !== -1) {
            _httpFunctional['Referer'] = 'https://www.netflix.com/';
        }
        if (typeof _httpFunctional['Origin'] === 'string'
            && _httpFunctional['Origin'].indexOf('iptv-ape.duckdns.org') !== -1) {
            _httpFunctional['Origin'] = 'https://www.netflix.com';
        }
        // 3) Keep-Alive proper RFC 7230 form (single-value sanitizer kept just
        //    "timeout=120" — but RFC wants "timeout=N, max=N"). Restore both.
        if (_httpFunctional['Keep-Alive'] === 'timeout=120'
            || /^timeout=\d+$/.test(_httpFunctional['Keep-Alive'] || '')) {
            _httpFunctional['Keep-Alive'] = 'timeout=300, max=1000';
        }
        // ──────────────────────────────────────────────────────────────────────
        // ──────────────────────────────────────────────────────────────────────

        // Emitir CAPA A — EXTHTTP funcional (cap ≤10KB universal)
        lines.push(`#EXTHTTP:${JSON.stringify(_httpFunctional)}`);

        // ── SUB-CAPA E: OVERFLOW — Todos los headers restantes en base64 ──
        // El Runtime Evasion Engine (ape-runtime-evasion-engine.js líneas 760-834)
        // decodifica este blob y los inyecta en cada HTTP request en runtime.
        try {
            const _httpOverflow = _buildOverflowHeaders(cfg, profile, index);
            // Merge Realtime Engine overflow si el cap 8KB los desplazó
            if (_realtimeOverflow) {
                Object.assign(_httpOverflow, _realtimeOverflow);
            }
            // Excluir keys que ya están en _httpFunctional para evitar duplicados
            for (const key of Object.keys(_httpFunctional)) {
                delete _httpOverflow[key];
            }
            if (Object.keys(_httpOverflow).length > 0) {
                const _overflowJson = JSON.stringify(_httpOverflow);
                const _overflowB64 = base64UrlEncode(_overflowJson);
                lines.push(`#EXT-X-APE-OVERFLOW-HEADERS:${_overflowB64}`);
            }
        } catch (eOverflow) {
            console.warn('[L2-OVERFLOW] Error building overflow headers:', eOverflow.message);
        }
        ...
        // Plan Supremo — capa de continuidad 206 (byterange resume tras microcortes)
        lines.push(`#KODIPROP:inputstream.adaptive.reconnect=true`);
        lines.push(`#KODIPROP:inputstream.adaptive.reconnect_byterange=true`);
        // AGENT-10 FIX 2026-04-12: emit stream_headers so Kodi ISA includes
        // User-Agent + Referer on every segment fetch. Critical for IPTV providers
        // that validate headers on segment requests (not just manifest).
        (function(){
            const _uaSafe = encodeURIComponent(_ua796 || 'VLC/3.0.20 LibVLC/3.0.20');
            const _refSafe = encodeURIComponent('https://www.google.com/');
            const _streamHdr = `User-Agent=${_uaSafe}&Referer=${_refSafe}`;
            lines.push(`#KODIPROP:inputstream.adaptive.stream_headers=${_streamHdr}`);
        })();
        ...
        lines.push(`#KODIPROP:inputstream.adaptive.video_codec=`);
        ...
        // ── USA v1.0: KODIPROP polimórficos ──
        ...
        // ⭐ OMEGA 5: Inyectar NIVEL_3 directivas del LAB por canal
        const _nivel3 = (typeof window !== 'undefined' && window.APE_PROFILES_CONFIG?.nivel3PerLayer) || {};
        for (const item of (_nivel3.VLC || [])) {
            const k = (item.key || '').trim(), v = (item.value || '').trim();
            if (k && v) lines.push(`#EXTVLCOPT:${k}=${v}`);
        }
        for (const item of (_nivel3.KOD || [])) {
            const k = (item.key || '').trim(), v = (item.value || '').trim();
            if (k && v) lines.push(`#KODIPROP:${k}=${v}`);
        }
        for (const item of (_nivel3.EI || [])) {
            const k = (item.key || '').trim(), v = (item.value || '').trim();
        ...
        lines.push(`#EXT-X-CMAF-CODEC-FALLBACK=`);
        ...
        lines.push(`#EXT-X-APE-LCEVC-FALLBACK:ENABLED=true,RESOLUTION=`);
        ...
        // AGENT-8 FIX 2026-04-12: Emit HTTP error fallback tree (was defined but
        // never invoked). Uses PRE_ARMED_RESPONSE_BUILDER.buildBlock to emit
        // consolidated FALLBACK-CHAIN + FALLBACK-STRATEGY + FALLBACK-PERSIST +
        // FALLBACK-GENOME-B64 (all 27 codes packed in base64 to avoid OTT Navigator
        // stream-variant count bug while keeping the data parseable).
        try {
            if (typeof PRE_ARMED_RESPONSE_BUILDER !== 'undefined' &&
                typeof PRE_ARMED_RESPONSE_BUILDER.buildBlock === 'function') {
                const _fallbackBlock = PRE_ARMED_RESPONSE_BUILDER.buildBlock({ channel, index });
                if (_fallbackBlock) {
                    _fallbackBlock.split('\n').filter(l => l.trim()).forEach(l => lines.push(l));
                }
            }
        } catch (_eFb) { /* silent */ }
        // ═══════════════════════════════════════════════════════════════════════
        // MASTER GUARANTEES TAG (2026-04-11) — 11 contratos auditados por PhDs
        // Enumera las garantías activas para auto-documentación de la lista.
        // Cada garantía tiene directivas M3U8 ejecutables asociadas (97% coverage).
        // ═══════════════════════════════════════════════════════════════════════
        lines.push(`#EXT-X-APE-GUARANTEES:V=11,AUDITED=2026-04-11,COVERAGE=97pct`);
        lines.push(`#EXT-X-APE-GUARANTEE-01:ID=ZERO_FREEZES,IMPL=VLCOPT-reconnect+HLSjs-stall1250+nudge3`);
        lines.push(`#EXT-X-APE-GUARANTEE-02:ID=ZERO_CUTS,IMPL=STREAM-INF-redundant-3urls+ABR-upFactor-0.7`);
        lines.push(`#EXT-X-APE-GUARANTEE-03:ID=ZERO_REBUFFER,IMPL=dynamic-buffer+cap-600s+memory-safe`);
        lines.push(`#EXT-X-APE-GUARANTEE-04:ID=ZERO_LAG,IMPL=START-offset-10s+catch-up-1.2-1.5x+sigmoid`);
        lines.push(`#EXT-X-APE-GUARANTEE-05:ID=ZERO_BUGS_CMCD,IMPL=CTA-5004-compliant-headers`);
        lines.push(`#EXT-X-APE-GUARANTEE-06:ID=MAX_ABR,IMPL=penalty-30s+realbitrate+no-throttle`);
        lines.push(`#EXT-X-APE-GUARANTEE-07:ID=MAX_IMAGE,IMPL=dvh1.08.06+BT2020+12bit+SMPTE2094`);
        lines.push(`#EXT-X-APE-GUARANTEE-08:ID=HTTP_ERROR_TREE,IMPL=27-codes-covered-3xx-4xx-5xx-CF520-527`);
        lines.push(`#EXT-X-APE-GUARANTEE-09:ID=PHD_407,IMPL=8-variants-basic-digest-ntlm-negotiate-bearer-fake-cf-squid`);
        lines.push(`#EXT-X-APE-GUARANTEE-10:ID=MULTI_PROVIDER,IMPL=parallel-x32+no-self-block+tls-permissive`);
        lines.push(`#EXT-X-APE-GUARANTEE-11:ID=SUPERAGENT_PLAYBACK,IMPL=audio-alternates+adaptive+http-retries-99`);
        lines.push(`#EXT-X-APE-GUARANTEE-SEAL:AGENTS=13,CHECKS-PASS=900+,PRODUCTION-STATUS=UNBLOCKED`);
        ...
        lines.push(`#EXT-X-APE-SPACE-VALIDATOR-CODECS:`);
        ...
        lines.push(`#EXT-X-APE-XTREAM-CODEC=`);
        ...
        lines.push(`#EXT-X-VNOVA-LCEVC-CODEC=`);
        ...
        lines.push(`#EXT-X-VNOVA-LCEVC-FALLBACK-RESOLUTION=`);
        ...
        lines.push(`#EXT-X-VNOVA-LCEVC-BASE-CODEC=`);
        ...
        lines.push(`#EXT-X-VNOVA-LCEVC-BASE-RESOLUTION=`);
        ...
        // ════════════════════════════════════════════════════════════════════════
        // ════════════════════════════════════════════════════════════════════════
        // L10 — HLS METADATA + SINGLE URL EMISSION
        // ════════════════════════════════════════════════════════════════════════
        // FIX 2026-04-17: Keep all HLS metadata tags (codec, bandwidth, resolution)
        // but remove URI= from EXT-X-MEDIA and EXT-X-I-FRAME-STREAM-INF to prevent
        // opening extra connections (was causing 509). Emit only 1 EXT-X-STREAM-INF
        // followed by 1 URL.

        // Audio rendition metadata (sin URI= para no abrir conexiones extra)
        ...
        // I-Frame info metadata (sin URI= para no abrir conexión)
        lines.push(`#EXT-X-I-FRAME-STREAM-INF:BANDWIDTH=${Math.round(_bw796*0.025)},CODECS="${_codec796}"`);

        // fMP4/CMAF Init Segment — Dynamic via Resolver (polimórfico + idempotente)
        // El URI apunta al resolver VPS que genera/cachea el init.mp4 POR CANAL usando ape_sid.
        // Al cambiar de canal, el player descarta el anterior y solicita el nuevo init del canal activo.
        // NOTA: NO apuntar a "init.mp4" estático (404). Usar el resolver como proxy dinámico.
        lines.push(`#EXT-X-MAP:URI="${primaryUrl.split('?')[0]}?ape_sid=${_sid796}&ape_nonce=${_nonce796}&seg=init.mp4&codec=${encodeURIComponent(_codec796)}"`);

        // Single stream variant + single URL
    ...
    function generateM3U8Stream(channels, options = {}) {
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                // ═══════════════════════════════════════════════════════════════
                // PASO 0: PRE-RESOLVER FRONTCDN REDIRECTS (antes de generar)
                // ═══════════════════════════════════════════════════════════════
                // FALLBACK DE GENERACIÓN: Construye la lista con las URLs CDN finales 
                // resolviendo los 302 originarios si VPS router no es preferido.
                try {
                    const frontCDNCount = channels.filter(ch => {
                        if (ch.type === 'movie' || ch.stream_type === 'movie' ||
                            ch.type === 'series' || ch.stream_type === 'series') return false;
                        const sid = ch.serverId || ch._serverId || ch.__serverKey;
                        let creds = sid && credentialsMap[sid] ? credentialsMap[sid] : credentialsMap['__current__'];
                        return creds && isFrontCDNHost(creds.baseUrl, creds);
                    }).length;

                    if (frontCDNCount > 0) {
                        console.log(`🛡️ [FrontCDN v3.0] Detectados ${frontCDNCount} canales FrontCDN — iniciando pre-resolución fallback...`);
                        if (useHUD) {
                            window.HUD_TYPED_ARRAYS.log(`🛡️ Pre-resolviendo ${frontCDNCount} FrontCDN...`, '#f59e0b');
                        }
                        const resolvedCount = await preResolveFrontCDNRedirects(channels, credentialsMap, (current, total) => {
                            if (useHUD) {
                                window.HUD_TYPED_ARRAYS.log(`🛡️ FrontCDN: ${current}/${total} resueltos`, '#f59e0b');
                            }
                        });
                        console.log(`🛡️ [FrontCDN v3.0] ✅ ${resolvedCount} URLs pre-resueltas`);
                    }
                } catch (e) {
                    console.warn(`🛡️ [FrontCDN v3.0] Error en pre-resolución: ${e.message}`);
                }
                ...
                for (let index = 0; index < channels.length; index++) {
                    ...
                    try {
                        ...
                        // URL-AUDIT FIX (2026-04-16): Skip channels with no valid URL
                        if (!entry) continue;
                        const chunk = entry + '\n\n';
                        ...
                        // Actualizar HUD
                        // Contador de perfil: SIEMPRE (O(1), sin throttle) para telemetría fiel.
                        if (useHUD && window.HUD_TYPED_ARRAYS && typeof window.HUD_TYPED_ARRAYS.tickProfile === 'function') {
                            window.HUD_TYPED_ARRAYS.tickProfile(profile);
                        }
                        // DOM pesado (barra + counter + líneas + speed): throttled cada 50 canales.
    ...
    async function generateM3U8(channels, options = {}) {
        ...
        if (typeof window !== 'undefined' && window.APEHealthRuntime && options.useAdmission !== false && typeof window.APEHealthRuntime.filterAdmittedChannels === 'function') {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 🎯 PREPUBLISH + Plan Supremo Moderno (buckets A/B/C)
    // buildCandidatesForChannel: construye variantes por canal para prepublish
    // o para emisión final. Ahora respeta modos de publicación:
    //   - strict_terminal: solo Bucket A (admission.url terminal 200)
    //   - balanced_95:    A + B (fallback panel 302) — default
    //   - diagnostic:     A + B + C con marcas
    // Canal sin admisión y sin creds → Bucket C (vacío, skipped).
    // URL Bucket A se publica TAL CUAL (no se le inyecta ?profile=) para no
    // romper firma/canonicidad del token ya validado por el health_checker.
    // ═══════════════════════════════════════════════════════════════════════════
    function buildCandidatesForChannel(channel, profiles, creds, admission, modeArg) {
        const streamId = String((channel && (channel.stream_id || channel.id)) || '').trim();
        // Modo: argumento > global window.APE_PUBLICATION_MODE > default 'balanced_95'.
        const mode = String(modeArg ||
                            (typeof window !== 'undefined' && window.APE_PUBLICATION_MODE) ||
                            'balanced_95').toLowerCase();
        const profileList = (Array.isArray(profiles) && profiles.length > 0) ? profiles : ['P1', 'P2', 'P3'];
        const result = {
            channel_id: streamId,
            channel_name: (channel && (channel.name || channel.tvg_name)) || '',
            variants: [],
            bucket: 'C',
            mode,
            skipped: true,
        ...
        if (!streamId) { result.reason = 'no_stream_id'; return result; }
        ...
        const roleFromUrl = (u) => {
            const path = String(u || '').toLowerCase().split('?')[0];
            if (path.endsWith('.m3u8')) return 'playlist_hls';
            if (path.endsWith('.ts'))   return 'segment_ts';
            if (path.endsWith('.mpd'))  return 'playlist_dash';
            if (path.endsWith('.m4s') || path.endsWith('.mp4')) return 'segment_cmaf';
            return 'unknown';
        };

        const admissionTier = admission && (admission.publicationTier || admission.publication_tier);
        const admissionUrl  = admission && admission.url;
        const admissionRole = admission && (admission.observedRole ||
            (function(adm){
                if (!adm || !adm.content_type) return null;
                const ct = String(adm.content_type).toLowerCase().split(';')[0].trim();
                if (ct === 'application/vnd.apple.mpegurl' || ct === 'application/x-mpegurl') return 'playlist_hls';
                if (ct === 'application/dash+xml') return 'playlist_dash';
                if (ct === 'video/mp2t') return 'segment_ts';
                if (ct === 'application/mp4' || ct === 'video/iso.segment') return 'segment_cmaf';
                return null;
            })(admission)
        );

        // ─── BUCKET A — admission.url terminal verificada ─────────────────────
        // Se publica TAL CUAL, una sola URL para todos los perfiles solicitados.
        // No se concatena ?profile= para preservar canonicidad del token firmado.
        if (admissionUrl && (admissionTier === 'A' || (!admissionTier && admissionRole))) {
            const canonical = canonicalize(String(admissionUrl));
            const roleByUrl = roleFromUrl(canonical);
            const expectedRole = (roleByUrl !== 'unknown') ? roleByUrl : (admissionRole || 'playlist_hls');
            for (const profile of profileList) {
                result.variants.push({
                    url: canonical,
                    profile,
                    expected_role: expectedRole,
                    expected_status: 'terminal_200',
                    publication_tier: 'A',
                    admission_source: 'health_checker'
                });
            }
            result.bucket = 'A';
            result.skipped = false;
            result.reason = 'admission_terminal';
            return result;
        }

        // ─── BUCKET B — fallback panel upstream (302 → 200) ────────────────────
        // Solo en modos balanced_95 y diagnostic. En strict_terminal se rechaza.
        if (mode === 'strict_terminal') {
            result.bucket = 'C';
            result.reason = 'strict_mode_requires_admission';
            return result;
        }
        if (!creds || !creds.baseUrl) {
            result.bucket = 'C';
            result.reason = 'no_admission_no_creds';
            return result;
        }
        const fallbackBase = `${creds.baseUrl}/live/${creds.username}/${creds.password}/${streamId}.m3u8`;
        for (const profile of profileList) {
            const sep = fallbackBase.includes('?') ? '&' : '?';
            const urlWithProfile = `${fallbackBase}${sep}profile=${encodeURIComponent(profile)}`;
            const canonical = canonicalize(urlWithProfile);
            result.variants.push({
                url: canonical,
                profile,
                expected_role: 'playlist_hls',
                expected_status: 'redirect_transition',
                publication_tier: 'B',
                admission_source: 'fallback_panel'
            });
        }
        result.bucket = 'B';
        result.skipped = false;
        result.reason = 'fallback_panel';
        return result;
    }
    ...
    if (typeof window !== 'undefined') {
        ...
        window.M3U8TypedArraysGenerator = {
            ...
            buildCandidatesForChannel: buildCandidatesForChannel,  // Etapa 3 plan "sin /resolve/"
    ...
    //          Mux Data QoE, Akamai Stream Validator, CMCD CTA-5004
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    const APE_PROFILE_MATRIX = {
      ...
      P0: {
        ...
        hlsjs: {
          ...
          abrBandWidthFactor: 0.98, abrBandWidthUpFactor: 0.70,
          abrMaxWithRealBitrate: true, abrSwitchInterval: 0,
          detectStallWithCurrentTimeMs: 1250, nudgeOffset: 0.1, nudgeMaxRetry: 3,
          fragLoadPolicy_maxTimeToFirstByteMs: 8000, fragLoadPolicy_maxNumRetry: 6,
          cmcd_useHeaders: true,
          nudgeMaxRetry: 5, nudgeOnVideoHole: true, liveSyncDurationCount: 3, liveMaxLatencyDurationCount: 10, maxLiveSyncPlaybackRate: 1.2, maxStarvationDelay: 6, errorPenaltyExpireMs: 30000,
        },
        exoplayer: {
        ...
        bitrateLadder: '150000000@7680x4320x120,@3840x2160x60,40000000@x60',
      },

      // ── P1: 4K SUPREME ─────────────────────────────────────────────────────────
      P1: {
        _meta: { name: '4K_SUPREME_REALTIME', bitrate: , resolution: '3840x2160', fps: 60, codec: 'hvc1.2.4.L153.B0,ec-3' },
        hlsjs: {
          ...
          fragLoadPolicy_maxTimeToFirstByteMs: 8000, fragLoadPolicy_maxNumRetry: 6,
          cmcd_useHeaders: true,
          nudgeMaxRetry: 3, nudgeOnVideoHole: true, liveSyncDurationCount: 3, liveMaxLatencyDurationCount: 10, maxLiveSyncPlaybackRate: 1.2, maxStarvationDelay: 4, errorPenaltyExpireMs: 30000,
        },
        exoplayer: {
        ...
        bitrateLadder: '@3840x2160x60,40000000@x60,20000000@x30',
      },
      ...
      P2: {
        ...
        hlsjs: {
          ...
          lowLatencyMode: true, enableWorker: true,
          abrEwmaFastLive: 3.0, abrEwmaSlowLive: 9.0,
          abrBandWidthFactor: 0.95, abrBandWidthUpFactor: 0.70,
          abrMaxWithRealBitrate: true, abrSwitchInterval: 0,
          detectStallWithCurrentTimeMs: 1250, nudgeOffset: 0.1, nudgeMaxRetry: 3,
          fragLoadPolicy_maxTimeToFirstByteMs: 8000, fragLoadPolicy_maxNumRetry: 6,
          cmcd_useHeaders: true,
          nudgeMaxRetry: 3, nudgeOnVideoHole: true, liveSyncDurationCount: 3, liveMaxLatencyDurationCount: 10, maxLiveSyncPlaybackRate: 1.3, maxStarvationDelay: 4, errorPenaltyExpireMs: 30000,
        },
        exoplayer: {
        ...
        bitrateLadder: '40000000@3840x2160x60,20000000@x60,10000000@1280x720x60',
      },

      // ── P3: FHD ADVANCED ───────────────────────────────────────────────────────
      P3: {
        _meta: { name: 'FHD_ADVANCED_REALTIME', bitrate: 20000000, resolution: '', fps: 60, codec: 'avc1.640028,mp4a.40.2' },
        hlsjs: {
        ...
        bitrateLadder: '20000000@x60,10000000@1280x720x60,5000000@1280x720x30',
      },
      ...
      P4: {
        ...
        hlsjs: {
          ...
          lowLatencyMode: false, enableWorker: true,
          abrEwmaFastLive: 3.0, abrEwmaSlowLive: 9.0,
          abrBandWidthFactor: 0.90, abrBandWidthUpFactor: 0.70,
          abrMaxWithRealBitrate: true, abrSwitchInterval: 0,
          detectStallWithCurrentTimeMs: 1250, nudgeOffset: 0.1, nudgeMaxRetry: 3,
          fragLoadPolicy_maxTimeToFirstByteMs: 10000, fragLoadPolicy_maxNumRetry: 4,
          cmcd_useHeaders: true,
          nudgeMaxRetry: 3, nudgeOnVideoHole: true, liveSyncDurationCount: 3, liveMaxLatencyDurationCount: 10, maxLiveSyncPlaybackRate: 1.5, maxStarvationDelay: 4, errorPenaltyExpireMs: 30000,
        },
        exoplayer: {
    ...
    function _buildCmcdHeaders(profileId, channelId) {
      // CTA-5004 CMCD — alineado exacto con HLS.js v1.6 (cmcd-controller.ts)
      // Reglas del spec (§3 + Annex A):
      // - Keys alfabetizadas por header
      // - Booleans bare (sin =true), omitir si false
      // - Strings quoted (sid, cid, nor)
      // - v=1 se OMITE (es default)
      // - ot: "v"=video-only, "a"=audio-only, "av"=muxed A+V, "m"=manifest, "i"=init, "tt"=subtitle
      // - HLS TS segments son muxed → ot=av (no "v")
      // - mtp va en CMCD-Request (NO en Session) per CTA-5004 Table 2
      // - br/tb en kbps | bl/d en ms | mtp = bandwidthEstimate/1000 kbps
      const p = APE_PROFILE_MATRIX[profileId];
      if (!p) return {};
      const br = Math.round(p._meta.bitrate / 1000);  // kbps
      const ladderTop = p.bitrateLadder ? parseInt(p.bitrateLadder.split(',')[0].split('@')[0]) : p._meta.bitrate;
      const tb = Math.round(ladderTop / 1000);  // kbps (top del ladder)
      const bl = p.exoplayer.minBufferMs;  // ms buffer length
      const mtp = Math.round(br * 1.5);  // kbps measured throughput estimado
      const sid = (typeof crypto !== 'undefined' && crypto.randomUUID)
        ? crypto.randomUUID()
        : Math.random().toString(36).slice(2, 18);
      const cid = String(channelId || 'ape-channel');
      return {
        // CMCD-Object: alfabético br,ot,tb (ot=av para muxed HLS TS)
        'CMCD-Object': `br=${br},ot=av,tb=${tb}`,
        // CMCD-Request: alfabético bl,mtp,su (mtp pertenece aquí per spec Table 2)
        'CMCD-Request': `bl=${bl},mtp=${mtp},su`,
        // CMCD-Session: alfabético cid,pr,sf,sid (sin mtp, v=1 omitido por default)
        'CMCD-Session': `cid="${cid}",pr=1,sf=h,sid="${sid}"`,
        // CMCD-Status: omitido cuando no hay starvation (bs se inyecta en runtime si ocurre)
      };
    }
    ...
    function _buildPlayerConfigHeaders(profileId) {
      ...
      return {
        'X-HLSjs-Config': JSON.stringify({
          ...
          maxMaxBufferLength: p.hlsjs.maxMaxBufferLength,
          backBufferLength: p.hlsjs.backBufferLength,
          maxBufferSize: p.hlsjs.maxBufferSize,
          maxBufferHole: p.hlsjs.maxBufferHole,
          ...
          abrSwitchInterval: p.hlsjs.abrSwitchInterval,
          detectStallWithCurrentTimeMs: p.hlsjs.detectStallWithCurrentTimeMs,
          nudgeOffset: p.hlsjs.nudgeOffset,
          nudgeMaxRetry: p.hlsjs.nudgeMaxRetry,
          nudgeOnVideoHole: p.hlsjs.nudgeOnVideoHole,
          maxStarvationDelay: p.hlsjs.maxStarvationDelay,
          maxLiveSyncPlaybackRate: p.hlsjs.maxLiveSyncPlaybackRate,
          liveSyncDurationCount: p.hlsjs.liveSyncDurationCount,
          liveMaxLatencyDurationCount: p.hlsjs.liveMaxLatencyDurationCount,
          errorPenaltyExpireMs: p.hlsjs.errorPenaltyExpireMs,
          // AGENT-6 FIX 2026-04-12: fragLoadPolicy params for hls.js retry chain
          fragLoadPolicy_maxTimeToFirstByteMs: p.hlsjs.fragLoadPolicy_maxTimeToFirstByteMs,
          fragLoadPolicy_maxNumRetry: p.hlsjs.fragLoadPolicy_maxNumRetry,
          // AGENT-6 FIX 2026-04-12: force-emit appendErrorMaxRetry with sensible per-profile default
          appendErrorMaxRetry: p.hlsjs.appendErrorMaxRetry ?? ({P0:6,P1:5,P2:4,P3:3,P4:3,P5:2}[profileId] ?? 3),
          manifestLoadPolicy_maxNumRetry: p.hlsjs.manifestLoadPolicy_maxNumRetry ?? p.hlsjs.fragLoadPolicy_maxNumRetry ?? 5,
        }),
    ...
    window.ApeRealtimeEngine = {
      ...
      diagnose() {
        console.group('[APE Realtime Engine v10.0 — Diagnostico]');
    ...
    } // END GUARD — APE REALTIME ENGINE v10.0
