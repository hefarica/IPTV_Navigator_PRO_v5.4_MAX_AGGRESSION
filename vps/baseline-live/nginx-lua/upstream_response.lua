do local _ct=(ngx.header["Content-Type"] or ""):lower(); if _ct:find("mpegurl",1,true) then ngx.header["Content-Length"]=nil end end -- APE-CL-FIX
-- ════════════════════════════════════════════════════════════════════════════
...
if not profile:match("^P[0-6]$") then profile = "P2" end
...
elseif profile == "P2" then
    ...
    upscaler = "AdaptiveSharpen"
    hdr_intent = "HDR_AUTO"
    virtual_4k = "ACTIVE"
    floor_lock = "ACTIVE"
    floor_bps = 5000000
elseif profile == "P3" then
    mapped_profile = "P3_HD_BOOST"
    upscaler = "FSRCNNX_x4"
    hdr_intent = "HDR_AUTO"
    virtual_4k = "ACTIVE"
    floor_lock = "ACTIVE"
    floor_bps = 8000000
elseif profile == "P4" then
    mapped_profile = "P4_HD_LITE"
    upscaler = "FSRCNNX_x4"
    hdr_intent = "HDR_FORCE"
    virtual_4k = "ACTIVE"
    floor_lock = "ACTIVE"
    floor_bps = 8000000
elseif profile == "P5" then
    mapped_profile = "P5_SD_RESCUE"
    upscaler = "FSRCNNX_x4"
    hdr_intent = "HDR_FORCE"
    virtual_4k = "ACTIVE"
    floor_lock = "ACTIVE"
    floor_bps = 12000000
elseif profile == "P6" then
    -- CRYSTAL EDGE: Máxima agresividad visual (China Box / Huawei 4K-8K)
    mapped_profile = "P6_CHINA_BOX_8K_ULTRA_AGGRESSIVE"
    upscaler = "HiSilicon_AI_SR_MAX"
    hdr_intent = "HDR_FORCE"
    virtual_4k = "ACTIVE"
    floor_lock = "ACTIVE"
    floor_bps = 15000000
...
if uri:find(".m3u8", 1, true) or uri:find(".m3u", 1, true) then
    ...
    -- CRYSTAL EDGE: Headers de máxima agresividad visual
    ngx.header["X-APE-Visual-Engine"]   = "CRYSTAL-UHD-v4.0"
    ngx.header["X-APE-Color-Space"]     = (profile == "P6") and "BT.2020/PQ" or "BT.709/SDR"
    ngx.header["X-APE-HDR-Profile"]     = (profile == "P6") and "HDR10+DolbyVision" or "SDR"
    ngx.header["X-APE-Crystal-Active"]  = (profile == "P6") and "1" or "0"
end
